...
| API service | API bound only to staging database, `APP_ENV=staging`, disjoint allowlists, polling disabled (2.2) | `staging-api` deploys from branch `staging`; last deployment succeeded on 2026-09-14; a public domain exists | Records described empty service identities only | Attest bindings under 2.3 |
