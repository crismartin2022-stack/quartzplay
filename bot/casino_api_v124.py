...
async def casino_juegos(refrescar: int = 0):
    """
    El catálogo para mostrar en el casino.

    Se devuelve ya resuelto: cada juego con el nombre de su marca y
    sus grupos, en vez de índices que el frontend tendría que cruzar.
    """
    datos = await _traer_juegos(forzar=bool(refrescar))
    marcas = datos.get("brands") or []
    grupos = datos.get("groups") or []

    salida = []
    for j in datos.get("games") or []:
        idx_marca = j.get("brand")
        marca = None
        if isinstance(idx_marca, int) and 0 <= idx_marca < len(marcas):
            marca = marcas[idx_marca].get("label")
        etiquetas = []
        for g in (j.get("group") or []):
            if isinstance(g, int) and 0 <= g < len(grupos):
                etiquetas.append(grupos[g].get("label"))
        salida.append({
            "id": j.get("id"),
            "titulo": j.get("title"),
            "imagen": j.get("image"),
            "marca": marca,
            "grupos": etiquetas,
            # Los grupos vienen como "platform:mobile": se separa para
            # poder filtrar sin que el frontend parsee texto.
            "movil": any("mobile" in (e or "") for e in etiquetas),
            "escritorio": any("desktop" in (e or "") for e in etiquetas),
        })

    return {
        "juegos": salida,
        "marcas": [m.get("label") for m in marcas],
        "total": len(salida),
        "actualizado": _fecha_local(
            datetime.fromtimestamp(_juegos_cache["ts"], tz=timezone.utc))
            if _juegos_cache["ts"] else None,
    }
...
async def casino_sesion(request: Request):
    ...
    if not (PROVEEDOR_URL and PROVEEDOR_CODE and PROVEEDOR_SECRET):
        raise HTTPException(503, "El casino no está configurado")
    ...
    async with pool.acquire() as conn:
        ...
        # El casino tiene que estar habilitado para su agencia
        permitidos = await _productos_de(conn, u["creado_por"])
        if "casino" not in permitidos:
            raise HTTPException(403,
                "Tu agencia no tiene el casino habilitado")
    ...
    headers = {**_firmar_proveedor(cuerpo),
               "Content-Type": "application/json"}

    async with httpx.AsyncClient(timeout=20) as client:
        r = await client.post(f"{PROVEEDOR_URL}/api/v1/session",
    ...
    if not datos.get("url"):
        # La respuesta completa, para saber si es un error suyo, una
        # moneda que no manejan, o un jugador que no reconocen.
            ...
            await conn.execute("""
                INSERT INTO casino_sesiones
                    (sesion, sesion_ext, user_id, agencia_code,
                     game_id, game_titulo, moneda, ultimo_uso)
                VALUES ($1,$2,$3,$4,$5,$6,$7,NOW())
                ON CONFLICT (sesion) DO UPDATE SET
                    sesion_ext = EXCLUDED.sesion_ext,
                    ultimo_uso = NOW()
            """, sesion_local, datos.get("session"), u["id"],
                 u["creado_por"], game_id, titulo,
                 (u["moneda"] or "ARS").upper())
    except Exception as e:
    ...
    return {
        "url": datos["url"],
        "sesion": datos.get("session") or sesion_local,
        "juego": game_id,
    }
