...
LIMITES = {
    ...
    "/api/agencias/login":    (8,  300),   # el admin entra por clave en
...
async def web_banners():
    ...
    async with pool.acquire() as conn:
        raw = await conn.fetchval(
            "SELECT valor FROM app_config WHERE clave='web_banners'")
    if not raw:
        return {"banners": []}
    try:
        datos = json.loads(raw)
    except Exception:
        log.warning("[WEB] web_banners no es JSON valido, se ignora")
        return {"banners": []}
    ...
    for b in datos[:8]:
        if not isinstance(b, dict):
            continue
        img = str(b.get("imagen") or "")[:500]
        if not img.startswith("https://"):
            continue   # solo https, para no romper el candado del navegador
        limpios.append({
            "imagen": img,
            "titulo": str(b.get("titulo") or "")[:80],
            "texto":  str(b.get("texto") or "")[:160],
            "link":   str(b.get("link") or "")[:300],
        })
    return {"banners": limpios}


@app.post("/api/admin/web/banners")
async def admin_web_banners(request: Request, _=Depends(auth.require_admin)):
    """Reemplaza la lista completa de banners del sitio."""
    body = await request.json()
    banners = body.get("banners")
    if not isinstance(banners, list):
        raise HTTPException(400, "Se espera una lista de banners")
    if len(banners) > 8:
        raise HTTPException(400, "Maximo 8 banners")
    limpios = []
    for b in banners:
        if not isinstance(b, dict):
            raise HTTPException(400, "Banner invalido")
        img = str(b.get("imagen") or "").strip()
        if not img.startswith("https://"):
            raise HTTPException(400, "Cada banner necesita una imagen https://")
        limpios.append({
            "imagen": img[:500],
            "titulo": str(b.get("titulo") or "")[:80],
            "texto":  str(b.get("texto") or "")[:160],
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO app_config (clave, valor, updated_at) VALUES ($1,$2,NOW())
            ON CONFLICT (clave) DO UPDATE SET valor=$2, updated_at=NOW()
        """, "web_banners", json.dumps(limpios, ensure_ascii=False))
    return {"ok": True, "total": len(limpios)}
