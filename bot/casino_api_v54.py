import os, re, time, hashlib, asyncio, hmac, json, logging, ast, secrets
...
async def admin_listar_limites(_=Depends(auth.require_admin)):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            ...
            ORDER BY l.alcance, l.moneda NULLS FIRST, l.agencia_code NULLS FIRST
        """)
    return {"limites": [{
    ...
    async with pool.acquire() as conn:
        if alcance == "global":
            ...
            await conn.execute(
                ...
                "AND COALESCE(moneda,'ARS')=$1", moneda)
            await conn.execute("""
    ...
    async with pool.acquire() as conn:
        ...
        filas = await conn.fetch("""
            ...
            WHERE b.status = 'pending'
              ...
              -- Solo la propia red. Antes se incluían los boletos sin
              -- agencia como "embudo", pero ver la reserva no hace que
              -- el cliente vaya a ese local, y ensucia los cierres con
              -- plata de clientes que no son de esa agencia.
              AND u.creado_por = ANY($1)
...
async def _sellar_huella(conn, code, request, es_live=False):
    ...
    try:
        ...
        await conn.execute("""
            UPDATE betslips SET ip=$2, user_agent=$3, device_hash=$4, es_live=$5
            WHERE code=$1
        """, code, ip, ua, dev, bool(es_live))
        # También en el usuario, para ver registros en cadena
        await conn.execute("""
...
async def auditar_cuotas(_=Depends(auth.require_admin)):
    ...
    try:
        cache = dict(_sr_all_cache) if "_sr_all_cache" in globals() else {}
    except Exception:
        cache = {}

    for _clave, entrada in list(cache.items()):
        try:
            data = entrada[0] if isinstance(entrada, tuple) else entrada
            deportes = data.get("sports", []) if isinstance(data, dict) else []
        except Exception:
            continue
...
async def riesgo_ips(dias: int = 30, minimo: int = 2, limite: int = 60,
    ...
    async with pool.acquire() as conn:
        filas = await conn.fetch("""
            ...
            ORDER BY COUNT(DISTINCT b.user_id) DESC, COUNT(*) DESC
            LIMIT $3
...
async def crear_apuesta(request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        ok_riesgo, motivo_riesgo = await _controlar_riesgo(conn, limpios, pot_win)
        if not ok_riesgo:
...
async def crear_usuario(request: Request,
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow("""
            INSERT INTO users
                (username, nombre_completo, documento, telefono, telegram_id,
                 balance, creado_por, moneda, password_hash, created_at)
...
async def all_markets():
    """
    Catálogo de prematch. La fuente ahora es Sportradar: el event_id que
    se guarda al apostar es el mismo que consulta la liquidación, que era
    justamente lo que fallaba con The Odds API.

    _armar_all_markets (la versión vieja) queda en el archivo por si hay
    que comparar, pero ya no se llama desde ningún lado.
    """
    return await _sr_all_markets_cacheado()
...
async def _cfg_riesgo(conn):
    ...
    return {
        # 0 = sin tope. En pesos.
        "max_evento": num("riesgo_max_evento", 0),
        "max_seleccion": num("riesgo_max_seleccion", 0),
...
async def _controlar_riesgo(conn, picks, potential_win):
    """Devuelve (ok, motivo). Se llama ANTES de debitar."""
    cfg = await _cfg_riesgo(conn)
    if not cfg["activo"]:
        return True, ""
...
async def admin_riesgo_config(request: Request, _=Depends(auth.require_admin)):
    ...
    vals = {
        ...
        "riesgo_max_seleccion": str(int(body.get("max_seleccion", 0) or 0)),
        "riesgo_max_evento": str(int(body.get("max_evento", 0) or 0)),
    }
    pool = await get_db()
    async with pool.acquire() as conn:
        for k, v in vals.items():
...
async def _sr_all_markets_cacheado():
    ...
    try:
        ...
        log.error(f"[SR] all-markets: {e}")
        return _sr_all_cache["data"] or {"sports": [], "fuente": "sportradar"}
