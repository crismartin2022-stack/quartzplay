...
async def admin_eventos(tipo: str = "prematch", deporte: str = "",
    ...
    if tipo == "live":
        ...
        # Prematch sale de Sportradar, que es la fuente actual. Antes
        # apuntaba a The Odds API, que dejamos de usar: por eso la
        # pantalla quedaba vacía.
        data = await _sr_all_markets_cacheado()
        if not (data or {}).get("sports"):
            # Respaldo: si Sportradar no devolvió nada, se intenta con
            # la fuente vieja antes que mostrar la pantalla en blanco.
            entrada = _football_cache.get("all_markets")
            data = entrada[0] if entrada else None
            if not data:
                data = await cache_swr("all_markets", ODDS_TTL_PREMATCH,
                                       _armar_all_markets)
    data = data or {}
