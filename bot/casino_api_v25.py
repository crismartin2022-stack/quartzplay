...
async def admin_escanear_combo(request: Request, _=Depends(auth.require_admin)):
    ...
    leidos = await leer_captura_con_claude(imagen, media_type)
    if not leidos:
        return {"ok": False, "mensaje": "No se pudieron leer apuestas en la imagen"}
...
async def leer_captura_con_claude(imagen_b64: str, media_type: str):
    ...
    Devuelve lista de picks: [{home, away, market, selection, odd}] o None.
    """
...
async def buscar_cuota_nuestra(home, away, market, selection):
    ...
    """
    catalogo = _football_cache.get("all_markets")
    if not catalogo:
...
def opciones_de_evento(ev):
    ...
    for k, v in (markets.get("totals", {}) or {}).items():
        etiqueta = k
        m = re.match(r"Over\s*([\d.]+)", k, re.I)
        if m: etiqueta = f"Más de {m.group(1)}"
        m = re.match(r"Under\s*([\d.]+)", k, re.I)
...
async def mejorar_combinada(request: Request):
    ...
    """
    body = await request.json()
    imagen = body.get("imagen","")          # base64 sin el prefijo data:
    media_type = body.get("media_type","image/jpeg")
    if not imagen:
        raise HTTPException(400, "Falta la imagen")
    if len(imagen) > 8_000_000:
        raise HTTPException(413, "La imagen es demasiado grande")

    leidos = await leer_captura_con_claude(imagen, media_type)
    if not leidos:
        return {"ok": False, "motivo": "no_se_leyo",
                "mensaje": "No pudimos leer apuestas en la imagen. "
