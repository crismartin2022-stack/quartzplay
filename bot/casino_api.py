...
async def _sr_odds_get(path):
    ...
    Los ':' de los IDs (sr:sport:1) se codifican a %3A."""
    if not SPORTRADAR_KEY:
        raise HTTPException(503, "Sportradar no configurado (falta SPORTRADAR_KEY)")
...
async def _sr_odds_mercados_dia(sport_id, fecha_iso):
    ...
    data = await _sr_odds_get(path)
    return data.get("sport_event_markets", []) or data.get("schedules", [])
...
async def sportradar_mercados_dia(_=Depends(auth.require_admin)):
    """Trae las cuotas de todos los eventos de fútbol de hoy (Daily Sport Event Markets)
    y muestra la estructura del primer evento con cuotas."""
    from datetime import date
    dia = date.today().strftime("%Y-%m-%d")
    try:
        eventos = await _sr_odds_mercados_dia("sr:sport:1", dia)
        if not eventos:
            return {"ok": False, "error": "Sin eventos con cuotas hoy", "fecha": dia}
        # Buscar el primer evento que tenga mercados
        con_cuotas = None
        for ev in eventos:
            if ev.get("markets"):
                con_cuotas = ev; break
        return {"ok": True, "fecha": dia, "total_eventos": len(eventos),
                "muestra": con_cuotas or eventos[0]}
    except HTTPException as e:
        return {"ok": False, "error": str(e.detail)}
    except Exception as e:
        return {"ok": False, "error": str(e)[:150]}
