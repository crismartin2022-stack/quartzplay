    ...
    async with pool.acquire() as conn:
        ...
        filas = await conn.fetch("""
            ...
            WHERE b.status = 'pending'
              ...
              AND (u.creado_por = ANY($1::text[]) OR u.creado_por IS NULL
...
async def _detalle_movimientos(conn, codes, desde, hasta, limite=200):
    ...
    filas = await conn.fetch("""
        ...
        FROM agencia_movimientos am
        LEFT JOIN users u ON u.id = am.user_id
        WHERE am.agencia_code = ANY($1::text[])
          AND am.created_at::date >= $2::date
          AND am.created_at::date <= $3::date
...
async def caja_agencia(desde: str = None, hasta: str = None,
    ...
    async with pool.acquire() as conn:
        ...
        if cliente:
            ...
            return {
                "desde": desde, "hasta": hasta, "moneda": yo["moneda"],
                "cliente": {
                    "id": u["id"],
...
async def caja_admin(desde: str = None, hasta: str = None,
    ...
    async with pool.acquire() as conn:
        ...
        codes = [r["code"] for r in rama]
        if not codes:
            return {"desde": desde, "hasta": hasta, "agencias": [],
        ...
        if cliente:
            ...
            if det is not None:
                det = [d for d in det if d["cliente_id"] == cliente]
            return {
                "desde": desde, "hasta": hasta, "moneda": u["moneda"] or "ARS",
                "cliente": {
                    "id": u["id"],
                    "nombre": u["nombre_completo"] or u["username"],
                    "saldo": int(u["balance"] or 0) // 100,
