...
async def _sr_odds_get(path):
    ...
    Ej: '/sports.json' o '/sports/sr:sport:1/2026-08-07/schedules.json'."""
    if not SPORTRADAR_KEY:
        raise HTTPException(503, "Sportradar no configurado (falta SPORTRADAR_KEY)")
    url = f"{SR_ODDS_BASE}/{SPORTRADAR_ACCESS}/v2/{SPORTRADAR_LANG}{path}"
    sep = "&" if "?" in url else "?"
    url = f"{url}{sep}api_key={SPORTRADAR_KEY}"
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.get(url)
    ...
    if r.status_code >= 400:
        ...
        raise HTTPException(502, f"Error consultando cuotas Sportradar ({r.status_code})")
    return r.json()
...
async def _sr_odds_eventos_dia(sport_id, fecha_iso):
    ...
    path = f"/sports/{sport_id}/{fecha_iso}/schedules.json"
    data = await _sr_odds_get(path)
    return data.get("sport_events", []) or data.get("schedules", [])


async def _sr_odds_mercados_evento(sport_event_id):
    """Mercados (cuotas) de un evento (Sport Event Markets)."""
    path = f"/sport_events/{sport_event_id}/markets.json"
...
async def sportradar_inspeccionar_cuotas(sport_id: str = "sr:sport:1", _=Depends(auth.require_admin)):
    """Prueba varias rutas del Daily Schedules del Odds Comparison Prematch
    y devuelve la respuesta cruda para ver la estructura real."""
    from datetime import date, timedelta
    hoy = date.today()
    intentos = []
    for delta in range(0, 4):
        dia = (hoy + timedelta(days=delta)).strftime("%Y-%m-%d")
        # Distintas variantes de la ruta de Daily Schedules
        rutas = [
            f"/sports/{sport_id}/schedules/{dia}/schedule.json",
            f"/sports/{sport_id}/{dia}/schedules.json",
            f"/schedules/{dia}/schedules.json",
        ]
        for ruta in rutas:
            try:
                ...
                # Buscar dónde están los eventos
                eventos = (data.get("sport_events") or data.get("schedules")
                           or data.get("sport_event") or [])
                intentos.append({
                    "ruta": ruta, "status": "OK", "claves": claves,
                    "cant_eventos": len(eventos) if isinstance(eventos, list) else 0,
                    "muestra": (eventos[0] if isinstance(eventos, list) and eventos else None),
                })
                if isinstance(eventos, list) and eventos:
                    return {"ok": True, "ruta_correcta": ruta, "fecha": dia,
                            "cant_eventos": len(eventos),
                            "muestra_evento": eventos[0],
                            "todas_las_claves": claves}
            except HTTPException as e:
                intentos.append({"ruta": ruta, "status": f"error {e.detail}"})
            except Exception as e:
                intentos.append({"ruta": ruta, "status": str(e)[:80]})
        # Solo probamos el primer día con todas las rutas, para no gastar requests
        if delta == 0 and any("OK" in str(i.get("status")) for i in intentos):
            break
    return {"ok": False, "intentos": intentos,
            "nota": "Ninguna ruta devolvió eventos. Revisar las claves que sí respondieron."}


@app.get("/api/admin/sportradar/partidos-hoy")@app.get("/api/admin/sportradar/partidos-hoy")
...
async def partido_stats(home: str, away: str, fecha: str = ""):
    """Stats en vivo/finales de un partido (por nombre de equipos).
    Público, lo usan app, agencia y box."""
...
async def partido_previa(home: str, away: str, fecha: str = ""):
