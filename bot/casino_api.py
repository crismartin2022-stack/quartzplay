...
async def admin_clientes(agencia: str = "", buscar: str = "",
    ...
    cond, args = [], []
    if agencia:
        args.append(agencia.upper()); cond.append(f"u.agencia_code=${len(args)}")
    if buscar:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(f"""
            SELECT u.id, u.nombre_completo, u.username, u.balance,
                   u.agencia_code, u.created_at, a.name AS agencia_nombre
            FROM users u
            LEFT JOIN agencias a ON a.code = u.agencia_code
            {where}
...
async def admin_crear_cliente(request: Request, _=Depends(auth.require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow("""
            INSERT INTO users (username, nombre_completo, telefono,
                               balance, agencia_code, created_at)
            VALUES ($1,$2,$3,0,$4,NOW())
            RETURNING id
        """, uname, nombre, telefono, agencia)
