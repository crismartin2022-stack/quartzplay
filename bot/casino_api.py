...
async def sportradar_inspeccionar_cuotas(sport_id: str = "sr:sport:1", _=Depends(auth.require_admin)):
    """Trae eventos de fútbol con cuotas (hoy o próximos días) y muestra
    la estructura del primer evento para mapear los mercados."""
    from datetime import date, timedelta
    hoy = date.today()
    for delta in range(0, 4):
        dia = (hoy + timedelta(days=delta)).strftime("%Y-%m-%d")
        try:
            data = await _sr_odds_get(f"/sports/{sport_id}/{dia}/schedules.json")
        except Exception:
            continue
        eventos = data.get("sport_events", []) or data.get("schedules", [])
        if not eventos:
            continue
        # Tomar el primero y traer sus mercados
        primer = eventos[0]
        ev = primer.get("sport_event", primer)
        eid = ev.get("id")
        try:
            mkts = await _sr_odds_mercados_evento(eid)
        except Exception as e:
            mkts = {"error": str(e)[:100]}
        return {"ok": True, "fecha": dia, "total_eventos": len(eventos),
                "primer_evento_id": eid,
                "estructura_evento": ev,
                "estructura_mercados": mkts}
    return {"ok": False, "error": "No se encontraron eventos con cuotas en los próximos días"}
...
async def sportradar_test_cuotas(_=Depends(auth.require_admin)):
    ...
    Prueba varios endpoints posibles del producto Odds Comparison."""
    from datetime import date
    hoy = date.today().strftime("%Y-%m-%d")
    # Distintos productos de cuotas de Sportradar tienen distintas rutas.
    # Probamos las más comunes (prematch odds / odds comparison).
    al = SPORTRADAR_ACCESS
    lg = SPORTRADAR_LANG
    # Ruta oficial del Odds Comparison Prematch v2 (según la doc)
    intentos = [
