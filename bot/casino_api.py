...
async def sportradar_inspeccionar_cuotas(sport_id: str = "sr:sport:1", _=Depends(auth.require_admin)):
    """Prueba las rutas de Daily Sport Event Markets / Schedules del
    Odds Comparison Prematch v2. Para al primer OK para no gastar el límite."""
    from datetime import date
    dia = date.today().strftime("%Y-%m-%d")
    # Rutas candidatas según la doc (Daily Schedules y Daily Sport Event Markets)
    rutas = [
        f"/sports/{sport_id}/schedules/{dia}/summaries.json",
        f"/sports/{sport_id}/schedules/{dia}/sport_event_markets.json",
        f"/sports/{sport_id}/{dia}/summaries.json",
        f"/sports/{sport_id}/{dia}/sport_event_markets.json",
    ]
    intentos = []
    for ruta in rutas:
        try:
            ...
            # Buscar dónde vienen los eventos
            eventos = None
            ...
            if eventos:
                return {"ok": True, "ruta_correcta": ruta, "claves": claves,
                        "cant_eventos": len(eventos),
                        "muestra_evento": eventos[0]}
            intentos.append({"ruta": ruta, "status": "OK pero sin eventos", "claves": claves})
        except HTTPException as e:
            intentos.append({"ruta": ruta, "status": str(e.detail)})
            # Si es 429, parar para no seguir gastando
            if "límite" in str(e.detail):
                break
        except Exception as e:
