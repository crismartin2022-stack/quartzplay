...
async def admin_crear_bono(request: Request, _=Depends(auth.require_admin)):
    ...
    pc = _fl(body.get("porcentaje"))
    tope = _num(body.get("tope"))
    rollover = _fl(body.get("rollover")) or 0
        ...
        if bid:
            await conn.execute("""
                UPDATE bonos SET nombre=$2, tipo=$3, monto_fijo=$4, porcentaje=$5,
                    tope=$6, rollover=$7, activo=$8 WHERE id=$1
...
async def _procesar_rollover(conn, user_id, monto_apostado):
    ...
    monto_apostado en pesos."""
    monto_cent = int(monto_apostado * 100)
    # Descontar del rollover pendiente global
    await conn.execute("""
        UPDATE users
    ...
    otorgados = await conn.fetch("""
        SELECT id, rollover_objetivo, rollover_cumplido, monto
        FROM bonos_otorgados WHERE user_id=$1 AND estado='activo'
        ORDER BY otorgado_at ASC
    """, user_id)
