...
async def telegram_onboarding(request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            INSERT INTO users (telegram_id, nombre_completo, telefono, email, balance)
            VALUES ($1, $2, $3, $4, 0)
...
async def confirmar_match(request: Request):
    ...
    async with pool.acquire() as conn:
        cli = await conn.fetchrow(
            "SELECT id, balance, nombre_completo FROM users WHERE id=$1 AND telegram_id < 0", user_id)
        if not cli:
...
async def canjear_vinculo(request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        if v["tipo"] == "cliente":
            ...
            cli = await conn.fetchrow("SELECT id, balance, telegram_id, nombre_completo FROM users WHERE id=$1", user_id)
            if not cli:
                raise HTTPException(404, "Cliente no encontrado")
            # ¿Ya existe un usuario con ese telegram_id? (cuenta que el cliente usó en el bot)
            tg_user = await conn.fetchrow("SELECT id, balance FROM users WHERE telegram_id=$1", tg_id)
            async with conn.transaction():
                if tg_user and tg_user["id"] != user_id:
...
async def quien_soy(request: Request):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            SELECT id, username, first_name, balance, telefono, nombre_completo
...
async def admin_crear_usuario(request: Request, _=Depends(auth.require_admin)):
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow("""
            ...
            RETURNING id
        """, base_user, nombre, doc, tel, fake_tg, dueno)
    return {"id": row["id"], "nombre": nombre, "documento": doc,
            "creado_por": dueno, "saldo": 0, "creado": True}
    ...
    async with pool.acquire() as conn:
        ...
        if monto > 0:
            await avisar_cliente(conn, user_id,
                f"💰 Te cargaron ${monto:,.0f} en tu cuenta QuartzPlay.\n"
...
async def cashout_ejecutar(code: str, request: Request):
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            ...
            # Marcar la apuesta como cerrada por cash out
            await conn.execute("""
                UPDATE betslips
                SET status='cashed_out', potential_win=$2, resultado='cashout'
                WHERE code=$1
            """, code, valor)
            # Registrar el movimiento (columnas reales: type, amount, method, status)
            try:
                await conn.execute("""
    ...
    return {"ok": True, "valor": valor, "saldo": (saldo or 0) / 100, "code": code}
