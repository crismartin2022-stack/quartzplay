...
async def admin_hist_movimientos(agencia: str = "", desde: str = "",
                                 hasta: str = "", export: str = "",
                                 _=Depends(auth.require_admin)):
    """Todos los movimientos de saldo (cc + clientes). Filtra por fecha/agencia."""
    pool = await get_db()
    async with pool.acquire() as conn:
        cond, args = ["1=1"], []
        if agencia:
        ...
        if desde:
            args.append(desde); cond.append(f"created_at::date >= ${len(args)}")
        if hasta:
            args.append(hasta); cond.append(f"created_at::date <= ${len(args)}")
        ...
        """, *args)
    datos = [{
        "agencia": r["agencia_code"], "contra": r["contra_code"],
        "tipo": r["tipo"], "monto": float(r["monto"] or 0),
        "saldo": float(r["saldo_luego"] or 0), "detalle": r["detalle"],
        "por": r["creado_por"],
        "fecha": r["created_at"].strftime("%d/%m/%Y %H:%M") if r["created_at"] else "",
    } for r in rows]
    if export == "csv":
        lineas = ["fecha,agencia,tipo,monto,saldo,contra,detalle,por"]
        for d in datos:
            lineas.append(f'{d["fecha"]},{d["agencia"]},{d["tipo"]},{d["monto"]},'
                          f'{d["saldo"]},{d["contra"] or ""},'
...
async def admin_hist_apuestas(agencia: str = "", desde: str = "",
                              hasta: str = "", export: str = "",
                              _=Depends(auth.require_admin)):
    """Historial de todas las apuestas. Filtra por fecha/agencia."""
    pool = await get_db()
    async with pool.acquire() as conn:
        cond, args = ["1=1"], []
        if agencia:
        ...
        if desde:
            args.append(desde); cond.append(f"b.created_at::date >= ${len(args)}")
        if hasta:
            args.append(hasta); cond.append(f"b.created_at::date <= ${len(args)}")
        rows = await conn.fetch(f"""
            SELECT b.code, b.stake, b.odd_total, b.potential_win, b.status,
                   b.created_at, u.nombre_completo, u.creado_por
            FROM betslips b
...
async def agencia_hist_movimientos(desde: str = "", hasta: str = "",
    ...
    """Movimientos de la rama de la agencia (ella y sus sub)."""
    pool = await get_db()
    async with pool.acquire() as conn:
        rama = await codes_de_la_rama(conn, agencia_code)
...
async def crear_combo_admin(request: Request, _=Depends(auth.require_admin)):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            ...
            VALUES ('admin', 'admin', $1, $2, $3, true, $4, $5, $6, $7, $8)
            RETURNING id
        """, nombre, str(limpios), round(odd_total, 2), agencias,
             dest_box, dest_app, dest_agencia, fuente)
    return {"id": row["id"], "odd_total": round(odd_total, 2),
            "agencias": agencias or "todas",
            ...
            await conn.execute("""
                INSERT INTO betslips
                    (code, user_id, picks, stake, odd_total,
                     potential_win, status, cliente_nombre,
                     created_at, expires_at)
                VALUES ($1, NULL, $2, 0, $3, 0, 'pending', $4,
                        NOW(), NOW() + interval '24 hours')
            """, code, str(limpios), odd_total, cliente)
