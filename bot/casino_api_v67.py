...
async def bloquear_cuotas_peligrosas(request: Request,
    ...
    async with pool.acquire() as conn:
        for p in res["problemas"]:
            if p["severidad"] not in niveles:
                continue
            if solo and p["event_id"] not in solo:
                continue
            if not p["event_id"]:
                continue
            ya = await conn.fetchval("""
                SELECT 1 FROM eventos_bloqueos
                WHERE objeto='evento' AND event_id=$1 AND alcance='global'
...
async def admin_eventos(tipo: str = "prematch", deporte: str = "",
    ...
    for sport in data.get("sports", []):
        ...
        if deporte and (sport.get("key") or "") != deporte:
            continue
        if liga and liga.lower() not in (sport.get("name") or "").lower():
            continue
    ...
    for sp in data.get("sports", []):
        catalogo.append({
            ...
            "nombre": sp.get("name"),
            "icon": sp.get("icon"),
            "eventos": len(sp.get("events") or []),
        })
    catalogo.sort(key=lambda x: -(x["eventos"] or 0))
