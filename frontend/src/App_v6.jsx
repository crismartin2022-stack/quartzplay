...
function useUsuario(){
  ...
  useEffect(()=>{
    ...
    // Diagnóstico visible para depurar el reconocimiento
    window.__qpDiag = {
      hayTelegram: !!window.Telegram,
      hayWebApp: !!tg,
      initDataLen: initData.length,
      tgUser: tg?.initDataUnsafe?.user?.id || null,
    };
    if(!initData){
      window.__qpDiag.resultado = "sin initData";
    ...
    fetch(`${API}/api/me`,{
      ...
      .then(r=>{ window.__qpDiag.apiStatus = r.status; return r.ok?r.json():null; })
      .then(d=>{ window.__qpDiag.apiResp = d; setUser({cargando:false, ...(d||{autenticado:false})}); })
      .catch(e=>{ window.__qpDiag.apiError = String(e); setUser({cargando:false, autenticado:false}); });
  },[]);
  return user;
}
...
function ScreenHome({ user, onNav, onBet, refCode }){
  ...
  return(
    <div style={{padding:"14px 12px 20px"}}>
      <div style={{textAlign:"center",color:Q.cyan,fontSize:10,marginBottom:8,
        fontFamily:"'Space Grotesk',system-ui",opacity:0.7}}>QuartzPlay · v7 ⚡</div>
      <div style={{background:"rgba(0,0,0,0.4)",border:`1px solid ${Q.border}`,
        borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:9,
        fontFamily:"monospace",color:Q.muted,whiteSpace:"pre-wrap",wordBreak:"break-all"}}>
        DIAG: {JSON.stringify(window.__qpDiag||{},null,1)}</div>
      {/* Tarjeta de saldo */}
      <GCard glow={Q.violet} style={{padding:18,marginBottom:14,
        ...
        {saldo!=null?(
          <div style={{color:Q.text,fontWeight:900,fontSize:32,marginTop:2,
            fontFamily:"'Space Grotesk',system-ui"}}>{ars(saldo)}
            <span style={{fontSize:14,color:Q.muted,fontWeight:400}}> ARS</span></div>
        ):autenticado?(
          <div style={{marginTop:6}}>
            <div style={{color:Q.text,fontWeight:900,fontSize:32,
              fontFamily:"'Space Grotesk',system-ui"}}>{ars(0)}
              <span style={{fontSize:14,color:Q.muted,fontWeight:400}}> ARS</span></div>
  ...
  return(
    <div style={{padding:"14px 12px 20px"}}>
      <GCard glow={Q.violet} style={{padding:18,marginBottom:14,
        ...
        {saldo!=null?(
          <div style={{color:Q.green,fontWeight:900,fontSize:26,marginTop:6,
            fontFamily:"'Space Grotesk',system-ui"}}>{ars(saldo)}<span style={{fontSize:12,color:Q.muted,fontWeight:400}}> ARS</span></div>
        ):(
        ...
        <GCard key={i} style={{padding:"10px 13px",marginBottom:6}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{color:(m.monto||0)>=0?Q.green:Q.red,fontWeight:700,fontSize:13,
              fontFamily:"'Space Grotesk',system-ui"}}>{(m.monto||0)>=0?"+":""}{ars(m.monto)}</div>
          </div>
        </GCard>
      ))}
      ...
      <div style={{color:Q.dim,fontSize:11,textAlign:"center",marginTop:16,lineHeight:1.5,
        fontFamily:"'Space Grotesk',system-ui"}}>
        Para cargar saldo, retirar o vincular tu cuenta de mostrador,
        contactá a tu agencia.</div>
    </div>
  );
}
