...
// ═══════════════════════════════════════════════════════════════
function ScreenMejorar({ onAction, user }){
  ...
  const elegir=(e)=>{
    ...
    rd.onload=()=>setImagen({b64:rd.result.split(",")[1],
  ...
  return(
    <div style={{background:Q.void,minHeight:"100%",padding:"14px 12px"}}>
      <BotMsg time="">
        ...
        {!imagen&&(
          <div style={{display:"flex",gap:8}}>
            <label style={{flex:1,border:`2px dashed ${Q.border}`,borderRadius:12,
              padding:"24px 10px",textAlign:"center",cursor:"pointer"}}>
            ...
            <label style={{flex:1,border:`2px dashed ${Q.border}`,borderRadius:12,
              ...
              <div style={{color:Q.text,fontWeight:700,fontSize:11,
                fontFamily:"'Space Grotesk',system-ui"}}>Galería</div>
            </label>
          </div>
        )}
        ...
        {imagen&&(
          <div>
            <img src={imagen.preview} alt="captura" style={{width:"100%",
              borderRadius:10,marginBottom:10,maxHeight:200,objectFit:"contain",
              background:"#000"}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setImagen(null);setRes(null);}}
                disabled={analizando} style={{flex:1,background:"transparent",
                border:`1px solid ${Q.border}`,borderRadius:10,padding:"11px",
                cursor:"pointer",color:Q.muted,fontSize:13,
                fontFamily:"'Space Grotesk',system-ui"}}>Cambiar</button>
              <button onClick={analizar} disabled={analizando} style={{flex:2,
                background:analizando?"rgba(255,255,255,0.06)"
                  :`linear-gradient(135deg,${Q.cyan},${Q.violet})`,
                border:"none",borderRadius:10,padding:"11px",
                cursor:analizando?"wait":"pointer",
                color:analizando?Q.muted:"#fff",fontWeight:700,fontSize:13,
                fontFamily:"'Space Grotesk',system-ui"}}>
            ...
            {res.picks_ok>0&&(
              <GCard glow={Q.green} style={{padding:14,marginTop:6}}>
                ...
                <div style={{color:Q.muted,fontSize:10,marginTop:8,lineHeight:1.4,
                  fontFamily:"'Space Grotesk',system-ui"}}>
                  Para jugarla, armá estos partidos desde Prematch. Pronto vas a
                  poder cargarla directo desde acá.
                </div>
              </GCard>
            )}
          </div>
  ...
  return(
    <div style={{maxWidth:520,margin:"0 auto",
      ...
      <div style={{flex:1,minHeight:0,overflowY:"auto",overflowX:"hidden",
        ...
        {screen==="mejorar"   &&<ScreenMejorar      onAction={handle} user={user}/>}
      </div>
    </div>
  );
}
