...
function Cierres({ agencia, onSesionExpirada }){
  ...
  const cargarMovs=async()=>{
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/historial/movimientos?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
  ...
  const cargarApuestas=async()=>{
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/historial/apuestas?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
  ...
  const cargar=async()=>{
    ...
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/cierre?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(!r.ok) throw new Error(`Error ${r.status}`);
      setRes(await r.json());
  ...
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[["resumen","📊 Resumen"],["movs","💸 Movs"],["apuestas","🎫 Apuestas"]].map(([k,l])=>(
        ...
        <div>
          ...
          <GCard glow={Q.green} style={{padding:16,marginBottom:12,
            ...
            <div style={{color:Q.muted,fontSize:10,textTransform:"uppercase",
              ...
              fontFamily:"'Space Grotesk',system-ui"}}>
              Total de mi rama ({res.moneda})</div>
          ...
          {/* Desglose por agencia de la rama */}
