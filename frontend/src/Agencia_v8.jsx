...
function Clientes({ agencia, onSesionExpirada }){
  ...
  const [verImpresiones,setVerImpresiones]=useState(false);
  ...
  return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        ...
        <Btn label="🖨️" onClick={()=>setVerImpresiones(true)} color={Q.amber} size="sm"/>
      ...
      {verImpresiones&&<ImpresionesRama agencia={agencia}
        onCerrar={()=>setVerImpresiones(false)} onSesionExpirada={onSesionExpirada}/>}
    </div>
  );
}
...
function Cierres({ agencia, onSesionExpirada }){
  ...
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[["resumen","📊 Resumen"],["movs","💸 Movs"],["apuestas","🎫 Apuestas"],["combos","⚡ Combos"]].map(([k,l])=>(
