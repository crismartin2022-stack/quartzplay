...
function CrearComboAgencia({ agencia, onVolver, onSesionExpirada }){
  ...
  const guardar=async()=>{
    ...
    try{
      ...
      if(!r.ok){ const e=await r.json().catch(()=>({}));
        throw new Error(e.detail||`Error ${r.status}`); }
      setMsg("✅ Combo creado. Ya aparece en tus terminales.");
      setPicks([]); setNombre("");
          ...
          <div style={{display:"flex",justifyContent:"space-between",
            ...
            <div style={{minWidth:0,flex:1}}>
              ...
              <div style={{color:Q.muted,fontSize:11}}>
                {c.picks.length} picks · {fmt(c.odd_total)}x</div>
            </div>
            <button onClick={()=>borrar(c.id)} style={{background:`${Q.red}18`,
              border:`1px solid ${Q.red}`,borderRadius:8,padding:"6px 12px",
...
function InfluencersAgencia({ agencia, onSesionExpirada }){
  ...
  const [vista,setVista]=useState("mios");   // mios | costos
  ...
  useEffect(()=>{ cargar(); if(vista==="costos") cargarCostos();
  // eslint-disable-next-line
  },[desde,hasta,vista]);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[["mios","🌟 Míos"],["costos","💸 Lo que me cuestan"]].map(([k,l])=>(
          <button key={k} onClick={()=>setVista(k)} style={{flex:1,
            background:vista===k?`${Q.violet}33`:"rgba(255,255,255,0.04)",
            border:`1px solid ${vista===k?Q.violet:Q.border}`,borderRadius:9,
            padding:"9px",cursor:"pointer",color:vista===k?Q.cyan:Q.muted,
...
function CrearSubAgencia({ agencia, onListo, onSesionExpirada }){
  const [form,setForm]=useState({name:"",username:"",password:"",pct_ggr:"",pct_ventas:""});
  const [msg,setMsg]=useState(""); const [proc,setProc]=useState(false);
...
function InfluencerCombosIA({ agencia, onSesionExpirada }){
  ...
  useEffect(()=>{
    fetch(`${API_URL}/api/influencer/me/combos-ia`,{headers:authHeaders(agencia.token)})
              ...
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  ...
                  {c.es_ia&&<span style={{background:`${Q.violet}33`,border:`1px solid ${Q.violet}`,
                    borderRadius:10,padding:"1px 7px",fontSize:9,color:Q.violet2,marginLeft:6}}>🤖 IA</span>}</div>
        ...
        <GCard key={c.id} style={{padding:"12px 14px",marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
            <div style={{minWidth:0,flex:1}}>
              ...
              <div style={{color:Q.muted,fontSize:10}}>{c.picks.length} picks · {c.fecha}</div>
            </div>
            <div style={{color:Q.green,fontWeight:700,fontSize:15,
              fontFamily:"'Space Grotesk',system-ui"}}>{fmt(c.odd)}x</div>
          </div>
          <div style={{display:"flex",gap:6,marginTop:10}}>
            <button onClick={()=>copiar(textoCombo(c,ref,
              `https://t.me/quartzplay_bot?start=${ref}_c${c.id}`),`txt${c.id}`)}
              style={{flex:1,background:`${Q.green}18`,border:`1px solid ${Q.green}`,borderRadius:8,
              padding:"8px",cursor:"pointer",color:Q.green,fontSize:11,fontWeight:700,
              fontFamily:"'Space Grotesk',system-ui"}}>
              {copiado===`txt${c.id}`?"✅":"📝 Texto"}</button>
            <button onClick={()=>descargarPlacaCombo(c,ref,
              `https://t.me/quartzplay_bot?start=${ref}_c${c.id}`)}
              style={{flex:1,background:`${Q.gold}18`,border:`1px solid ${Q.gold}`,borderRadius:8,
              padding:"8px",cursor:"pointer",color:Q.gold,fontSize:11,fontWeight:700,
              fontFamily:"'Space Grotesk',system-ui"}}>🖼️ Placa</button>
          </div>
        </GCard>
      ))}
    </div>
...
function CrearComboInfluencer({ agencia, onListo, onSesionExpirada }){
  ...
  const [picks,setPicks]=useState([{home:"",away:"",sel:"",odd:""}]);
  const [msg,setMsg]=useState("");
  const [proc,setProc]=useState(false);

  const setPick=(i,k,v)=>setPicks(ps=>ps.map((p,j)=>j===i?{...p,[k]:v}:p));
  const addPick=()=>setPicks(ps=>[...ps,{home:"",away:"",sel:"",odd:""}]);
  const delPick=(i)=>setPicks(ps=>ps.filter((_,j)=>j!==i));

  const guardar=async()=>{
    const limpios=picks.filter(p=>p.home&&p.sel&&p.odd);
    if(limpios.length===0){ setMsg("Cargá al menos una selección completa"); return; }
    setProc(true); setMsg("");
    try{
      const r=await fetch(`${API_URL}/api/influencer/me/combos`,{
        method:"POST",headers:{"Content-Type":"application/json",...authHeaders(agencia.token)},
        body:JSON.stringify({nombre,codigo,picks:limpios.map(p=>({
  ...
  return(
    <GCard glow={Q.violet} style={{padding:16,marginBottom:14}}>
      <div style={{color:Q.violet2,fontWeight:700,fontSize:14,marginBottom:12,
        fontFamily:"'Space Grotesk',system-ui"}}>⚡ Nuevo combo</div>
      ...
      <input value={codigo} onChange={e=>setCodigo(e.target.value.toUpperCase())}
        placeholder="Código de salida (ej: JUANTIPS)"
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
          borderRadius:9,padding:"9px 12px",color:Q.text,fontSize:16,marginBottom:12,
          fontFamily:"'Space Grotesk',system-ui"}}/>
      {picks.map((p,i)=>(
        <div key={i} style={{border:`1px solid ${Q.border}`,borderRadius:10,padding:10,marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{color:Q.muted,fontSize:10,fontFamily:"'Space Grotesk',system-ui"}}>Selección {i+1}</span>
            {picks.length>1&&<span onClick={()=>delPick(i)} style={{color:Q.red,fontSize:11,
              cursor:"pointer"}}>quitar</span>}
          </div>
          <div style={{display:"flex",gap:6,marginBottom:6}}>
            <input value={p.home} onChange={e=>setPick(i,"home",e.target.value)} placeholder="Local"
              style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
                borderRadius:8,padding:"8px",color:Q.text,fontSize:14,
                fontFamily:"'Space Grotesk',system-ui"}}/>
            <input value={p.away} onChange={e=>setPick(i,"away",e.target.value)} placeholder="Visitante"
              style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
                borderRadius:8,padding:"8px",color:Q.text,fontSize:14,
                fontFamily:"'Space Grotesk',system-ui"}}/>
          </div>
          <div style={{display:"flex",gap:6}}>
            <input value={p.sel} onChange={e=>setPick(i,"sel",e.target.value)} placeholder="Apuesta (ej: Gana local)"
              style={{flex:2,background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
                borderRadius:8,padding:"8px",color:Q.text,fontSize:14,
                fontFamily:"'Space Grotesk',system-ui"}}/>
            <input value={p.odd} onChange={e=>setPick(i,"odd",e.target.value)} placeholder="Cuota"
              inputMode="decimal"
              style={{width:70,background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
                borderRadius:8,padding:"8px",color:Q.text,fontSize:14,
                fontFamily:"'Space Grotesk',system-ui"}}/>
          </div>
        </div>
      ))}
      <button onClick={addPick} style={{width:"100%",background:"transparent",
        border:`1px dashed ${Q.border}`,borderRadius:9,padding:"9px",cursor:"pointer",
        color:Q.muted,fontSize:12,marginBottom:10,
        fontFamily:"'Space Grotesk',system-ui"}}>+ Otra selección</button>
      <Btn label={proc?"GUARDANDO...":"Crear combo"} onClick={guardar} color={Q.violet} full disabled={proc}/>
      {msg&&<div style={{fontSize:12,marginTop:8,
        color:msg.startsWith("✅")?Q.green:Q.red,
        fontFamily:"'Space Grotesk',system-ui"}}>{msg}</div>}
    </GCard>
