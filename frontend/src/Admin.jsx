...
function TabDiag({ adminKey, onNoAutorizado }){
  ...
  const inspeccionar=async()=>{
    ...
    try{
      const r=await fetch(`${API}/api/admin/sportradar/inspeccionar-cuotas`,{headers:adminHeaders(adminKey)});
                  ...
                  <div style={{marginTop:8}}>
                    ...
                    {srDeportes&&!srDeportes.cargando&&srDeportes.ok&&(
                      <div style={{marginTop:6,maxHeight:160,overflowY:"auto"}}>
                        ...
                        <Btn label="🔬 Inspeccionar estructura de cuotas" onClick={inspeccionar}
                          outline color={Q.cyan} size="sm" full/>
                        {srEstruct&&!srEstruct.cargando&&(
                          <pre style={{color:Q.muted,fontSize:8,whiteSpace:"pre-wrap",
                            wordBreak:"break-all",maxHeight:200,overflowY:"auto",marginTop:6,
                            background:"rgba(0,0,0,0.3)",padding:8,borderRadius:6}}>
                            {srEstruct.ok
                              ? JSON.stringify(srEstruct.estructura_mercados,null,1).slice(0,1500)
                              : "❌ "+srEstruct.error}</pre>
                        )}
                      </div>
                    )}
