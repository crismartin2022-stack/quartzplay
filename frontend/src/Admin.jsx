...
function ProveedoresCasino({ adminKey, onNoAutorizado }){
  ...
  return(
    <div>
        ...
        <div key={p.marca} style={{display:"flex",
          ...
          <div style={{minWidth:0,flex:1}}>
            <div style={{color:p.activo?Q.text:Q.dim,fontSize:12.5,
              fontWeight:600,fontFamily:"'Space Grotesk',system-ui"}}>
      ...
      {/* Dónde se muestra cada proveedor. Uno puede convenir en el
          sitio y no en la app: los juegos de escritorio se ven
          cortados en pantalla chica y el jugador cree que falla la
          app, no que el juego no era para ahí. */}
      {!agSel&&lista.length>0&&(
        <div style={{marginTop:16,paddingTop:14,
          borderTop:`1px solid ${Q.border}`}}>
          <div style={{color:Q.text,fontSize:12.5,fontWeight:700,
            marginBottom:4,fontFamily:"'Space Grotesk',system-ui"}}>
            Dónde se muestra cada uno</div>
          <div style={{color:Q.dim,fontSize:10.5,marginBottom:10,
            lineHeight:1.5,fontFamily:"'Space Grotesk',system-ui"}}>
            Las mesas en vivo aparecen en la app aunque el proveedor
            las marque solo para escritorio.</div>

          {lista.filter(p=>p.activo).map(p=>(
            <div key={p.marca} style={{display:"flex",
              justifyContent:"space-between",alignItems:"center",gap:8,
              padding:"8px 0",borderTop:`1px solid ${Q.border}`}}>
              <span style={{color:Q.text,fontSize:11.5,minWidth:0,flex:1,
                overflow:"hidden",textOverflow:"ellipsis",
                whiteSpace:"nowrap",
                fontFamily:"'Space Grotesk',system-ui"}}>
                {p.es_vivo&&"🎥 "}{p.marca}</span>
              <div style={{display:"flex",gap:5,flexShrink:0}}>
                ...
                <button onClick={()=>superficie(p.marca,"en_web",!p.en_web)}
                  ...
                  style={{background:p.en_web?`${Q.green}14`:"transparent",
                    ...
                    cursor:"pointer"}}>Web</button>
                {!p.es_vivo&&(
                  <button onClick={()=>superficie(p.marca,"forzar_movil",
                      !p.forzar_movil)}
                    disabled={proc}
                    title="Mostrar en la app aunque no esté marcado para móvil"
