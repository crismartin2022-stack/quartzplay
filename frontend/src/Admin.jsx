...
function TabDiag({ adminKey, onNoAutorizado }){
  ...
  const verMercadosDia=async()=>{
    setSrDia({cargando:true});
    try{
      const r=await fetch(`${API}/api/admin/sportradar/mercados-dia`,{headers:adminHeaders(adminKey)});
      setSrDia(await r.json());
    }catch(e){ setSrDia({ok:false,error:"Sin conexión"}); }
  };
                  ...
                  <div style={{marginTop:8}}>
                    ...
                    {srDeportes&&!srDeportes.cargando&&srDeportes.ok&&(
                      <div style={{marginTop:6,maxHeight:160,overflowY:"auto"}}>
                        ...
                        <div style={{height:6}}/>
                        <Btn label="💰 Ver cuotas del día (todos los partidos)"
                          onClick={verMercadosDia} color={Q.gold} size="sm" full/>
                        {srDia&&!srDia.cargando&&(
                          <pre style={{color:Q.gold,fontSize:8,whiteSpace:"pre-wrap",
                            ...
                            {srDia.ok
                              ? `${srDia.total_eventos} eventos\n\n`+JSON.stringify(srDia.muestra,null,1).slice(0,2500)
                              : "❌ "+srDia.error}</pre>
                        )}
                        ...
                        {srEstruct&&srEstruct.ok&&srEstruct.muestra_evento&&(
