  ...
  const cargar=async()=>{
    try{
      ...
      if(ra.ok){ const d=await ra.json();
        const lista=[]; const walk=(n)=>{ if(!n)return;
          if(n.code) lista.push({code:n.code,name:n.name||n.code});
          (n.hijos||n.children||[]).forEach(walk); };
        if(Array.isArray(d.arbol)) d.arbol.forEach(walk); else walk(d.arbol||d);
        setAgencias(lista);
      }
    }catch(e){}
  };
  ...
  return(
    <div style={{background:Q.void,minHeight:"100vh",
      ...
      <div style={{padding:"16px",maxWidth:620,margin:"0 auto",
        ...
        {tab==="limites"  &&<TabLimites   adminKey={adminKey} onNoAutorizado={onLogout}/>}
