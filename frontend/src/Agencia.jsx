...
class CazaError extends Component {
  ...
  render(){
    if(this.state.err){
      return (
        <div style={{padding:SPACING[20],background:"#1a0000",minHeight:"100vh",color:"#fff",
          ...
          <button onClick={()=>this.setState({err:null})}
            style={{background:"#7C3AED",border:"none",borderRadius:RADII.md,padding:"12px 20px",
...
function AlertaError({ mensaje, critico=false }){
  ...
  return(
    <div style={{
      ...
      display:"flex", alignItems:"flex-start", gap:SPACING[12],
    }}>
      ...
      <div>
        ...
        <div style={{color:critico?Q.text:Q.red,fontSize:critico?13:12,
          fontFamily:F_BODY,lineHeight:1.4}}>{mensaje}</div>
      </div>
    </div>
  );
}
...
<div class="ft">iaqp.com · ${agencia.code}</div>
</body></html>`;
  ...
  return(
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,
      ...
      <span style={{color:"#fff",fontWeight:900,fontSize:size*0.36,
        fontFamily:F_BODY,lineHeight:1}}>
        {iniciales||"?"}
      </span>
    </div>
  );
...
function EstadisticasPartidoBoton({ ev }){
  ...
  const abrir=()=>{
    ...
    fetch(`${API_URL}/api/partido/stats?${q}`).then(r=>r.json()).then(setStats).catch(()=>setStats({disponible:false}));
  };
      ...
      {abierto&&(
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(2,2,8,0.92)",
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:Q.void,
            ...
            maxWidth:520,margin:"0 auto",width:"100%",maxHeight:"80vh",overflowY:"auto",
            padding:"20px 16px 32px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <span style={{color:Q.text,fontWeight:700,fontSize:14,
                fontFamily:F_BODY}}>{home} vs {away}</span>
              <button onClick={()=>setAbierto(false)} style={{background:"transparent",border:"none",
                color:Q.muted,fontSize:26,cursor:"pointer"}}>×</button>
            </div>
...
function FiltroEventos({ ligas, liga, setLiga, busqueda, setBusqueda, total }){
  return(
    <div style={{marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:SPACING[4],overflowX:"auto",paddingBottom:SPACING[4],
...
function FlujoCodigo({ agencia, onSesionExpirada }){
  ...
  // Reservas esperando pago: el cajero las ve llegar sin que el cliente
  // tenga que dictarle el código. Se refresca sola.
  const [reservas,setReservas]=useState([]);

  const cargarReservas=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/reservas`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok){ const d=await r.json(); setReservas(d.reservas||[]); }
    }catch(e){}
  };

  useEffect(()=>{
    cargarReservas();
    const t=setInterval(cargarReservas,20000);
  ...
  return(
    <div>
      ...
      {/* Reservas esperando pago. Tocar una la carga en el buscador,
          así el cajero no tiene que tipear el código. Desaparecen
          solas al cobrarse o al vencer. */}
      {!slip&&reservas.length>0&&(
        <GCard style={{padding:SPACING[16],marginBottom:12}}>
          ...
          {reservas.map(r=>(
            <div key={r.code} onClick={()=>{ setCode(r.code); setErr(""); }}
              style={{display:"flex",justifyContent:"space-between",
                alignItems:"center",gap:SPACING[8],padding:"8px 4px",cursor:"pointer",
              ...
              <div style={{minWidth:0,flex:1}}>
                ...
                <div style={{color:Q.muted,fontSize:12,marginTop:2,
                  ...
                  whiteSpace:"nowrap"}}>
                  {r.terminal&&(
                    <span style={{color:Q.cyan,marginRight:4}}><Monitor size={12}/></span>
                  )}
      ...
      {slip&&step==="confirmar"&&slip.status==="cashout_pending"&&(
        <GCard glow={Q.gold} style={{padding:SPACING[20],marginBottom:12}}>
      ...
      {slip&&step==="confirmar"&&slip.status!=="cashout_pending"&&(
        <div>
          <GCard glow={Q.green} style={{padding:SPACING[20],marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",
              alignItems:"center",marginBottom:14}}>
              <div>
                <div style={{color:Q.green,fontWeight:900,fontSize:18,
                  fontFamily:F_BODY}}>{slip.code}</div>
            ...
            <div style={{marginTop:14,paddingTop:SPACING[12],borderTop:`1px solid ${Q.violet}44`}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{color:Q.muted,fontSize:13}}>Cuota total</span>
                <span style={{color:Q.gold,fontWeight:700,fontSize:16,
                  fontFamily:F_BODY}}>{fmt(slip.odd_total)}x</span>
              </div>
              {slip.stake>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                ...
                <span style={{color:Q.text,fontWeight:700,fontSize:15,
                  fontFamily:F_BODY}}>{ars(slip.stake)}</span>
              </div>}
              {slip.potential_win>0&&<div style={{display:"flex",justifyContent:"space-between"}}>
                ...
                <span style={{color:Q.green,fontWeight:900,fontSize:22,
                  fontFamily:F_BODY}}>{ars(slip.potential_win)}</span>
              </div>}
            </div>
          </GCard>
          ...
          <GCard style={{padding:SPACING[16],marginBottom:12}}>
            <div style={{color:Q.text,fontWeight:700,fontSize:13,marginBottom:12,
              fontFamily:F_BODY}}>¿Qué quiere hacer el cliente?</div>
            <div style={{display:"flex",flexDirection:"column",gap:SPACING[8]}}>
              <button onClick={confirmarApuesta} style={{
                background:"rgba(255,255,255,0.04)",border:`1px solid ${Q.green}`,
                borderRadius:RADII.lg,padding:SPACING[16],cursor:"pointer",textAlign:"left"}}>
      ...
      {step==="retiro"&&retiro&&(
        <GCard glow={Q.gold} style={{padding:SPACING[20],marginBottom:12}}>
          <div style={{color:Q.gold,fontWeight:800,fontSize:16,marginBottom:6,
            fontFamily:F_BODY}}><Banknote size={16}/> Retiro para pagar</div>
          <div style={{color:Q.muted,fontSize:13,marginBottom:16}}>
          ...
          {retiro.estado!=="pendiente"?(
            <div style={{color:Q.amber,fontSize:13,textAlign:"center",
              fontFamily:F_BODY}}>Este retiro ya está {retiro.estado}</div>
          ):(
            <div style={{display:"flex",gap:SPACING[8]}}>
      ...
      {step==="retiro_ok"&&retiro&&(
        <GCard glow={Q.green} style={{padding:SPACING[24],textAlign:"center",marginBottom:12}}>
      ...
      {step==="done_pagado"&&slip&&(
        <>
          {slip.aviso_tope&&(
            <GCard glow={Q.amber} style={{padding:SPACING[16],marginBottom:10,
...
function FlujoManual({ agencia }){
  ...
  const [bbFeature,setBbFeature]=useState({bet_builder:false,bb_max_picks:4});

  useEffect(()=>{
    fetch(`${API_URL}/api/agencias/me/features`,{headers:authHeaders(agencia.token)})
  ...
  const togglePick=(ev,sport,label,odd)=>{
    ...
    setPicks(p=>{
      ...
      // Límite de selecciones del mismo partido
      const delMismo=p.filter(x=>x.home===ev.home&&x.away===ev.away).length;
      if(delMismo>=(bbFeature.bb_max_picks||4)){
        alert(`Máximo ${bbFeature.bb_max_picks||4} selecciones del mismo partido`);
        return p;
      }
      return[...w,{id,home:ev.home,away:ev.away,sel:label,odd,sport,
        event_id:ev.event_id||ev.id||null, sport_key:ev.sport_key||null,
        // Hora del partido: la anulación la usa para saber si el
        // evento está por empezar. Sin esto el boleto no se puede anular.
        commence_time:ev.commence_time||ev.time||""}];
    });
  };
  const hasPick=(ev,label)=>picks.some(x=>x.id===`${ev.home}-${ev.away}-${label}`);
  const totBruto=picks.length?picks.reduce((a,p)=>a*p.odd,1):1;

  // Detectar mismo partido y cotizar con el margen de correlación
  const partidos={};
  ...
  // Lista de selecciones desplegable, para poder sacar de a una
  const [verPicks,setVerPicks]=useState(false);
  useEffect(()=>{
    if(picks.length===0){ setCuotaProt(null); return; }
    let vivo=true;
    fetch(`${API_URL}/api/bet-builder/cotizar`,{
      method:"POST",headers:{"Content-Type":"application/json"},
  ...
  const reset=()=>{ setPicks([]); setMonto(montoInicial(agencia?.moneda)); setCliente(""); setCodigoInf(""); setStep("armar"); setSlip(null); };
    ...
    <div>
      <GCard style={{padding:SPACING[16],marginBottom:14}}>
        ...
        <input value={codigoInf} onChange={e=>setCodigoInf(e.target.value.toUpperCase())}
          ...
          style={{width:"100%",background:"rgba(255,255,255,0.05)",
            border:`1px solid ${codigoInf?Q.cyan:Q.border}`,borderRadius:RADII.md,padding:"12px 16px",
            color:Q.text,fontSize:14,fontFamily:F_BODY}}/>
      </GCard>
        ...
        <>
          {!liveDeportes&&(
            <GCard style={{padding:SPACING[20],textAlign:"center"}}>
              <div style={{color:Q.muted,fontSize:12,fontFamily:F_BODY}}>Cargando partidos en vivo...</div>
            </GCard>
          )}
          ...
          {liveDeportes&&liveDeportes
            ...
            .map(m=>(
            <GCard key={m.id} glow={Q.pink} style={{padding:SPACING[16],marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div style={{color:Q.text,fontWeight:600,fontSize:13,fontFamily:F_BODY}}>
                  {m.home} vs {m.away}
                </div>
              ...
              <div style={{display:"flex",gap:SPACING[4]}}>
                {[{label:m.home+" gana",odd:m.odds.L},
                  ...
                  <button key={opt.label} onClick={()=>togglePick({home:m.home,away:m.away,event_id:m.id,sport_key:m.sport_key,commence_time:m.commence_time||m.time||""},"Live",opt.label,opt.odd)} style={{
                    flex:1,
              ...
              {abiertos[m.id]&&(
                <MercadosEvento ev={m}
                  ...
                  onToggle={(e,label,odd)=>
                    togglePick({home:m.home,away:m.away,event_id:m.id,sport_key:m.sport_key,commence_time:m.commence_time||m.time||""},"En vivo",label,odd)}
                  color={Q.pink}/>
              )}
            </GCard>
          ))}
      ...
      {tabOferta==="prematch"&&(
        <>
          ...
          {prematchDeportes&&prematchDeportes.length===0&&(
            <GCard style={{padding:SPACING[24],textAlign:"center"}}>
              <div style={{marginBottom:8}}><Inbox size={28} color={Q.muted}/></div>
              <div style={{color:Q.muted,fontSize:12,
                fontFamily:F_BODY}}>
          ...
          {filtrarDeportes(prematchDeportes, liga, busqueda).map(d=>(
            <GCard key={d.name} style={{padding:SPACING[16],marginBottom:12}}>
              ...
              {(d.events||[]).map(ev=>(
                <div key={ev.id||(ev.h+ev.a)} style={{marginBottom:12,
                  borderBottom:`1px solid ${Q.dim}`,paddingBottom:SPACING[12]}}>
                  <div style={{display:"flex",justifyContent:"space-between",
                    marginBottom:6,gap:SPACING[8]}}>
                  ...
                  <div style={{display:"flex",gap:SPACING[4]}}>
                    {[{label:ev.h+" gana",odd:ev.odds?.L},
                      ...
                      <button key={opt.label}
                        onClick={()=>togglePick({home:ev.h,away:ev.a,event_id:ev.id,sport_key:ev.sport_key,commence_time:ev.commence_time||ev.time||""},d.name,opt.label,opt.odd)}
                        style={{
                  ...
                  {abiertos[ev.id]&&(
                    <MercadosEvento ev={ev}
                      ...
                      onToggle={(e,label,odd)=>
                        togglePick({home:ev.h,away:ev.a,event_id:ev.id,sport_key:ev.sport_key,commence_time:ev.commence_time||ev.time||""}, d.name, label, odd)}
                      color={Q.cyan}/>
                  )}
                  <EstadisticasPartidoBoton ev={{h:ev.h,a:ev.a,commence_time:ev.commence_time,event_id:ev.event_id||ev.id}}/>
                </div>
              ))}
            </GCard>
          ))}
      ...
      {picks.length>0&&(
        <div style={{position:"sticky",bottom:0,zIndex:20,
          ...
          <GCard glow={Q.violet} style={{padding:SPACING[16]}}>
            {/* Las selecciones cargadas, con botón para sacar cada una.
                Antes había que buscar la cuota en la lista y tocarla de
                nuevo para quitarla, que no era evidente. */}
            {verPicks&&(
              <div style={{marginBottom:10,paddingBottom:SPACING[12],
                borderBottom:`1px solid ${Q.border}`,maxHeight:180,
                overflowY:"auto"}}>
                {picks.map(p=>(
                  <div key={p.id} style={{display:"flex",
                    ...
                    <button onClick={()=>setPicks(x=>x.filter(y=>y.id!==p.id))}
                      aria-label="Quitar"
                      style={{background:"transparent",border:"none",
                        color:Q.red,fontSize:17,cursor:"pointer",
                        padding:"0 4px",flexShrink:0,lineHeight:1}}>×</button>
                  </div>
                ))}
                <button onClick={()=>{
                    if(window.confirm("¿Borrar todas las selecciones?"))
                      setPicks([]);
                  }} style={{width:"100%",marginTop:7,
                  background:"transparent",border:`1px solid ${Q.border}`,
                  borderRadius:RADII.sm,padding:"8px",cursor:"pointer",
            ...
            <div style={{display:"flex",justifyContent:"space-between",
              alignItems:"center",marginBottom:10}}>
              <div onClick={()=>setVerPicks(v=>!v)} style={{cursor:"pointer"}}>
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  ...
                  <span style={{color:Q.dim,fontSize:12,marginLeft:5}}>
                    {verPicks?"▲":"▼"}</span>
                  {mismoPartido&&<span style={{color:Q.gold,fontSize:12,marginLeft:6}}><Link size={11}/> mismo partido</span>}
                </div>
              ...
              <div style={{display:"flex",gap:SPACING[4]}}>
                {montosDe(agencia?.moneda).map(v=>(
                  <button key={v} onClick={()=>setMonto(v)} style={{
                    ...
                    borderRadius:RADII.md,padding:"4px 8px",cursor:"pointer",
...
function PagoLocalForm({ slip, onConfirm, onCancel, error, paying }){
  ...
  return(
    <GCard glow={error?Q.red:Q.amber} style={{padding:SPACING[20],marginBottom:12}}>
      <div style={{color:Q.amber,fontWeight:700,fontSize:15,marginBottom:4,
        fontFamily:F_BODY}}>Cobrar apuesta en efectivo</div>
      <div style={{color:Q.muted,fontSize:12,marginBottom:16}}>
...
function DoneScreen({ slip, titulo, color, tipo, onReset }){
  ...
  return(
    <GCard glow={color} style={{padding:SPACING[24],textAlign:"center"}}>
      <div style={{marginBottom:10}}>
        {color===Q.green?<Icon name="circle-check" size={52} color={color}/>:color===Q.amber?<Icon name="wallet-cards" size={52} color={color}/>:<PartyPopper size={52} color={color}/>}
      </div>
      ...
      <div style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${Q.border}`,
        borderRadius:RADII.lg,padding:SPACING[16],marginBottom:16,textAlign:"left"}}>
        {[
          ...
          <div key={l} style={{display:"flex",justifyContent:"space-between",
            padding:"4px 0",borderBottom:`1px solid ${Q.dim}`}}>
      ...
      <div style={{display:"flex",gap:SPACING[12],marginBottom:10}}>
        <button onClick={imprimir} style={{
          ...
          border:`1px solid ${printed?Q.border:Q.violet}`,borderRadius:RADII.lg,padding:SPACING[16],
          color:printed?Q.muted:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",
          fontFamily:F_BODY,textTransform:"uppercase",
        }}><Printer size={14}/> {printed?"Reimprimir":"Imprimir ticket"}</button>
      </div>
...
function CorregirPick({ pick, token, onAplicar, onQuitar, onSesionExpirada }){
  ...
  return(
    <div style={{marginTop:10,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`}}>
      ...
      <div style={{display:"flex",gap:SPACING[8],marginTop:8,alignItems:"center"}}>
      ...
      <div style={{display:"flex",gap:SPACING[12],marginBottom:10}}>
        ...
        <label style={{flex:1,border:`2px dashed ${Q.border}`,borderRadius:RADII.lg,
          ...
          <input type="file" accept="image/*" multiple onChange={elegirArchivo}
            style={{display:"none"}}/>
          <div style={{marginBottom:4}}><ImageIcon size={26}/></div>
          <div style={{color:Q.text,fontWeight:700,fontSize:12,
            fontFamily:F_BODY}}>
        ...
        <div>
          ...
          {res.faltan_picks&&(
            <div style={{background:`${Q.red}12`,border:`1px solid ${Q.red}66`,
              borderRadius:RADII.md,padding:"12px 12px",marginBottom:12,color:Q.red,
          ...
          {res.picks.map((p,i)=>{
            ...
            <GCard key={i} glow={p.odd_final?est.c:undefined}
              ...
              <div style={{display:"flex",gap:SPACING[12],fontSize:12,marginBottom:8,
                fontFamily:F_BODY}}>
              ...
              <button onClick={()=>setCorrigiendo(corrigiendo===i?null:i)} style={{
                ...
                fontSize:12,fontWeight:700,fontFamily:F_BODY}}>
                {corrigiendo===i?"✕ Cerrar":<><Pencil size={12}/> Está mal / Corregir</>}
              </button>
              {corrigiendo===i&&(
          ...
          {res.picks_ok<res.picks_total&&(
            <div style={{background:`${Q.amber}12`,border:`1px solid ${Q.amber}55`,
              borderRadius:RADII.md,padding:"12px 12px",marginBottom:12,color:Q.amber,
              fontSize:12,lineHeight:1.4,fontFamily:F_BODY}}>
          ...
          {res.picks_ok>0&&(
            <GCard glow={Q.green} style={{padding:SPACING[16],marginTop:4}}>
              <div style={{display:"flex",justifyContent:"space-between",
                marginBottom:12}}>
                <span style={{color:Q.muted,fontSize:12,
                  fontFamily:F_BODY}}>Cuota total nuestra</span>
        ...
        <div style={{display:"flex",alignItems:"center",gap:SPACING[8],marginBottom:8}}>
          ...
          <input type="number" inputMode="numeric" min="0" value={monto}
            ...
            style={{flex:1,minWidth:0,background:"rgba(255,255,255,0.05)",
              ...
              <div style={{display:"flex",justifyContent:"space-between",
                marginBottom:10,padding:"8px 0",borderTop:`1px solid ${Q.dim}`}}>
...
function CrearComboAgencia({ agencia, onVolver, onSesionExpirada }){
  ...
  // Combos ya creados, para verlos debajo del formulario
  ...
  return(
    <div>
      ...
      <div style={{color:Q.text,fontWeight:700,fontSize:15,marginBottom:10,
        fontFamily:F_BODY}}>Crear combo</div>
      ...
      {(destinatarios.subs.length>0||destinatarios.influencers.length>0)&&(
        <GCard style={{padding:SPACING[12],marginBottom:10}}>
            ...
            <div style={{marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <span style={{color:Q.muted,fontSize:12,textTransform:"uppercase",letterSpacing:1,
                  fontFamily:F_BODY}}><Building2 size={12}/> Sub-agencias</span>
      ...
      {deportes&&deportes.length>0&&(
        <div style={{display:"flex",gap:SPACING[4],overflowX:"auto",paddingBottom:SPACING[4],marginBottom:10}}>
          <button onClick={()=>setLiga(null)} style={{
      ...
      {!deportes&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
      ...
      {picks.length>0&&(
        <GCard glow={Q.violet} style={{padding:SPACING[16],marginBottom:10}}>
  ...
  return(
    <div>
      ...
      {combos&&combos.length===0&&(
        <GCard style={{padding:SPACING[24],textAlign:"center"}}>
          <div style={{marginBottom:8}}><Icon name="clipboard-list" size={26} color={Q.muted}/></div>
        ...
        return(
          <GCard key={c.id} glow={esCasa?Q.gold:undefined}
            ...
            <div onClick={()=>setAbierto(abierta?null:c.id)}
              style={{display:"flex",justifyContent:"space-between",
                alignItems:"center",gap:SPACING[8],cursor:"pointer"}}>
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontWeight:600,fontSize:13,
                  fontFamily:F_BODY}}>{c.nombre}
                  {esCasa&&<span style={{background:`${Q.gold}22`,border:`1px solid ${Q.gold}`,
                    borderRadius:RADII.md,padding:"4px 8px",fontSize:12,color:Q.gold,marginLeft:6}}><Icon name="landmark" size={11}/> De la casa</span>}</div>
                <div style={{color:Q.muted,fontSize:12}}>
                  ...
                  {montosDe(agencia?.moneda).slice(0,3).map(m=>(
                    <div key={m} style={{display:"flex",justifyContent:"space-between",padding:"4px 0"}}>
                      ...
                      <span style={{color:Q.green,fontWeight:700,fontSize:12,
                        fontFamily:F_BODY}}>→ {ars(Math.round(m*(c.odd_total||1)))}</span>
                    </div>
                  ))}
                </div>

                {/* Cobrar para un cliente */}
                <div style={{marginTop:12,paddingTop:SPACING[12],borderTop:`1px solid ${Q.violet}44`}}>
                  <input value={cliente} onChange={e=>setCliente(e.target.value)}
                    placeholder="Cliente (opcional)"
                    style={{width:"100%",background:"rgba(255,255,255,0.05)",
                      border:`1px solid ${Q.border}`,borderRadius:RADII.md,padding:"8px 12px",
                  ...
                  <div style={{display:"flex",gap:SPACING[8],marginBottom:8}}>
                    {montosDe(agencia.moneda).map(m=>(
                      <button key={m} onClick={()=>setMonto(m)} style={{flex:1,
                        ...
                        padding:"8px 4px",cursor:"pointer",color:monto===m?Q.gold:Q.muted,
                  ...
                  <Btn label={cobrando===c.id?"COBRANDO...":<><Icon name="wallet-cards" size={13}/> Cobrar {ars(monto)}</>}
  ...
  return(
    <div>
      ...
      {cargando&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
        fontFamily:F_BODY}}>Generando combos...</div>}

      {combos&&combos.length===0&&!cargando&&(
        <GCard style={{padding:SPACING[24],textAlign:"center"}}>
          <div style={{marginBottom:8}}><Moon size={26} color={Q.muted}/></div>
          <div style={{color:Q.muted,fontSize:13,
            fontFamily:F_BODY}}>
      ...
      {(combos||[]).map(combo=>{
        ...
        <GCard key={combo.id} glow={combo.tagColor}
          ...
          <div style={{display:"flex",justifyContent:"space-between",
            alignItems:"flex-start",marginBottom:8,gap:SPACING[8]}}>
            <div style={{minWidth:0,flex:1}}>
              <div style={{color:Q.text,fontWeight:700,fontSize:14,
          ...
          <button onClick={()=>setAbierto(abiertoEste?null:combo.id)} style={{
            width:"100%",background:"transparent",border:`1px solid ${Q.dim}`,
            borderRadius:RADII.md,padding:"8px",cursor:"pointer",color:Q.dim,fontSize:12,
  ...
  return(
    <div>
      <div style={{display:"flex",gap:SPACING[8],marginBottom:12}}>
        ...
        <Btn label={<Lock size={14} role="img" aria-label="Bloqueos"/>} onClick={()=>setVerBloqueos(true)} color={Q.red} size="sm"/>
        <Btn label="+ Nuevo" onClick={()=>setAlta(true)} color={Q.violet} size="sm"/>
      </div>

      <AlertaError mensaje={msg}/>

      {cargando&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[16],
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:Q.deep,
        ...
        <div style={{display:"flex",justifyContent:"space-between",
          ...
          <div style={{color:Q.text,fontWeight:700,fontSize:16,
            fontFamily:F_BODY}}><Printer size={16}/> Impresiones de mi rama</div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:Q.deep,
        ...
        <div style={{display:"flex",justifyContent:"space-between",
          alignItems:"center",marginBottom:16}}>
          <div style={{color:Q.text,fontWeight:700,fontSize:16,
            fontFamily:F_BODY}}><Lock size={16}/> Bloqueos de mi rama</div>
          <button onClick={onCerrar} style={{background:"transparent",border:"none",
            color:Q.muted,fontSize:26,cursor:"pointer",lineHeight:1}}>×</button>
        </div>
        {!data&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
        ...
        {(data||[]).map((b,i)=>{
          ...
          return(
            <GCard key={i} glow={esBloqueo?Q.red:Q.green} style={{padding:"12px 12px",marginBottom:6}}>
...
function AltaCliente({ agencia, onListo, onCancel, onSesionExpirada }){
  ...
  // Usuario y clave: son las credenciales con las que el cliente entra
  // al sitio web. Las elige el cajero al darlo de alta.
  ...
  const crear=async()=>{
    ...
    if(!usuario.trim()){ setMsg("El usuario es obligatorio"); return; }
    if(!/^[a-z0-9._-]{3,40}$/.test(usuario.trim().toLowerCase())){
      setMsg("El usuario admite letras, números, punto, guion y guion bajo (3 a 40)");
      return;
    }
    ...
    try{
      ...
      onListo({id:d.id,nombre:d.nombre,documento:d.documento,saldo:0,
        telefono:tel,username:d.username,tiene_telegram:false,
        de_esta_agencia:true});
  ...
  return(
    <div>
      ...
      <GCard glow={Q.violet} style={{padding:SPACING[20]}}>
        ...
        {[["Nombre y apellido",nombre,setNombre,"Juan Pérez","text"],
          ["Usuario para el sitio web",usuario,setUsuario,"juanperez","text"],
          ["Clave (mínimo 6)",clave,setClave,"••••••","text"],
        ...
        <div style={{color:Q.muted,fontSize:12,marginTop:10,lineHeight:1.5,
          ...
          en la agencia. Anotale los datos antes de cerrar.
        </div>
      </GCard>
    </div>
  );
...
function FichaCliente({ agencia, user, onVolver, onSesionExpirada }){
  ...
  return(
    <div>
      ...
      <GCard glow={Q.violet} style={{padding:SPACING[20],marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{minWidth:0,flex:1}}>
            ...
            <div style={{color:Q.muted,fontSize:12,marginTop:2}}>
              ...
              {user.tiene_telegram?<> · Telegram <Send size={11}/></>:""}
            </div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{color:Q.muted,fontSize:12,textTransform:"uppercase",
        ...
        {ficha?.bloqueado&&(
          <div style={{marginTop:10,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`,
            ...
            <Lock size={12}/> Bloqueado por {ficha.bloqueado_por||"—"}
      ...
      {/* Bloqueo */}
      {ficha&&(
        <GCard glow={ficha.bloqueado?Q.green:Q.red} style={{padding:SPACING[16],marginBottom:12}}>
          {ficha.bloqueado?(
    ...
    <GCard style={{padding:"12px 16px",marginBottom:12,
      ...
      <div style={{display:"flex",alignItems:"center",gap:SPACING[12],
        ...
        <button onClick={correr} disabled={proc} style={{
          ...
          border:"none",borderRadius:RADII.md,padding:"8px 16px",
...
function DetalleTicket({ code, onCerrar }){
  const [d,setD]=useState(null);
  const [err,setErr]=useState("");

  useEffect(()=>{
    let vivo=true;
    fetch(`${API_URL}/api/betslip/${encodeURIComponent(code)}`)
      .then(r=>r.ok?r.json():Promise.reject(new Error(`Error ${r.status}`)))
      .then(x=>vivo&&setD(x))
      .catch(e=>vivo&&setErr(e.message==="Failed to fetch"
  ...
  const ESTADOS={pending:"Reservada, sin pagar", active:"En juego",
    ...
    cashed_out:"Cerrada con cash out", cancelled:"Anulada"};
  ...
  return(
    <div onClick={onCerrar} style={{position:"fixed",inset:0,zIndex:300,
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:420,
        ...
        {d&&<>
          {/* Si está reservada, el código va grande: es lo que el
              cliente muestra en el mostrador para pagarla. */}
          {(d.status||"").toLowerCase()==="pending"&&(
            <CodigoReserva code={code} compacto/>
          )}
          <div style={{display:"flex",justifyContent:"space-between",
            alignItems:"baseline",marginBottom:10,marginTop:10,fontSize:12}}>
...
function TicketHistorial({ t, agencia, anular, onCambio, onSesionExpirada }){
  ...
  // Se muestra el botón solo mientras la ventana sigue abierta. El
  // servidor vuelve a validar igual: esto es comodidad, no seguridad.
  const minutosDesde=(()=>{
    const crudo=t.pagado_at||t.fecha_iso||t.creado_at;
    if(!crudo) return null;
    const d=new Date(crudo);
    if(isNaN(d)) return null;
    return (Date.now()-d.getTime())/60000;
  })();
  const dentroDeVentana = minutosDesde==null
    || minutosDesde <= (anular?.minutos_tras_pago ?? 5);
  const sePuedeAnular = anular?.puede_anular && !res && !pagado
    && !anulado && dentroDeVentana;
  ...
  return(
    <GCard glow={res==="ganada"&&!pagado?Q.green:undefined}
      ...
      <div style={{display:"flex",justifyContent:"space-between",
        alignItems:"center",gap:SPACING[8]}}>
        {/* Tocar los datos del ticket abre el detalle con las
            selecciones. Los botones de acción quedan aparte para que
            no se dispare sin querer al liquidar o pagar. */}
        <div onClick={()=>setVerDetalle(true)}
          style={{minWidth:0,flex:1,cursor:"pointer"}}>
          <div style={{display:"flex",alignItems:"center",gap:SPACING[8],marginBottom:3}}>
            <span style={{color:Q.cyan,fontWeight:700,fontSize:13,
              fontFamily:F_BODY}}>{t.code}</span>
            <span style={{color:Q.dim,fontSize:12}}>ver ›</span>
            <span style={{background:`${colorEstado}22`,
              border:`1px solid ${colorEstado}`,borderRadius:RADII.xl,padding:"4px 8px",
        ...
        <div style={{marginTop:10,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`}}>
          ...
          {res==="ganada"&&(
            <button disabled={proc} onClick={pagar} style={{
              ...
              border:"none",borderRadius:RADII.md,padding:"12px",cursor:"pointer",
          ...
          {confirmando&&(
            <div style={{marginTop:8,padding:"12px 12px",
              ...
              <input value={motivo} onChange={e=>setMotivo(e.target.value)}
                ...
                style={{width:"100%",background:"rgba(255,255,255,0.05)",
                  border:`1px solid ${Q.dim}`,borderRadius:RADII.md,padding:"8px 12px",
              ...
              <div style={{display:"flex",gap:SPACING[8]}}>
                <button disabled={proc||!motivo.trim()}
                  ...
                  border:"none",borderRadius:RADII.md,padding:"8px",
...
// Los avisos que bajan del admin. Rota solo entre los que hay, y
// los urgentes no se pueden descartar sin leerlos: son los que
// importan de verdad.
function CarruselAvisos({ agencia, onSesionExpirada }){
  ...
  const cargar=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/avisos`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada&&onSesionExpirada(); return; }
      if(r.ok){
        const nuevos=(await r.json()).avisos||[];
        // Suena solo si hay uno urgente sin ver que antes no estaba
        const urgenteNuevo=nuevos.some(a=>a.nivel==="urgente"&&!a.visto
          &&!avisos.some(v=>v.id===a.id));
        if(urgenteNuevo) sonarAlerta(true);
        setAvisos(nuevos);
      }
    }catch(e){}
  };
  ...
  useEffect(()=>{
    cargar();
    // Cada 40 segundos: un aviso urgente que tarda dos minutos en
    // aparecer llega tarde para lo que suele avisarse.
    const t=setInterval(cargar,40000);
    return()=>clearInterval(t);
    // eslint-disable-next-line
  },[]);

  // Rota cada 6 segundos si hay más de uno
  useEffect(()=>{
    if(avisos.length<2||abierto) return;
    const t=setInterval(()=>setI(x=>(x+1)%avisos.length),6000);
    return()=>clearInterval(t);
  },[avisos.length,abierto]);

  const marcarVisto=async(id)=>{
    try{
      await fetch(`${API_URL}/api/agencias/me/avisos/${id}/visto`,
        {method:"POST",headers:authHeaders(agencia.token)});
      setAvisos(a=>a.map(x=>x.id===id?{...x,visto:true}:x));
    }catch(e){}
  };

  if(!avisos.length) return null;
  const a=avisos[Math.min(i,avisos.length-1)];
  const COLOR={info:Q.cyan, aviso:Q.gold, urgente:Q.red};
  const c=COLOR[a.nivel]||Q.cyan;

  return(
    <div onClick={()=>{setAbierto(v=>!v); if(!a.visto) marcarVisto(a.id);}}
      style={{background:`${c}12`,border:`1px solid ${c}55`,
        borderRadius:RADII.md,padding:"12px 12px",marginBottom:12,
        cursor:"pointer"}}>
      <div style={{display:"flex",justifyContent:"space-between",
          ...
          <div style={{color:c,fontWeight:700,fontSize:12.5,
            ...
            {!a.visto&&(
              <span style={{background:c,color:"#000",borderRadius:RADII.sm,
                padding:"4px 4px",fontSize:12,marginLeft:6,
        ...
        {avisos.length>1&&(
          <div style={{display:"flex",gap:SPACING[4],flexShrink:0,paddingTop:SPACING[4]}}>
            {avisos.map((_,k)=>(
...
// ── Soporte: los clientes que escribieron ─────────────────────
// La IA resuelve lo repetitivo. Acá llegan los que necesitan a una
// persona: reclamos de plata y todo lo que la IA no pudo cerrar.
function SoporteAgencia({ agencia, onSesionExpirada }){
  ...
  const [sonido,setSonido]=useState(()=>{
    try{ return localStorage.getItem("qp_sop_sonido")!=="0"; }
    catch(e){ return true; }
  });
  ...
  const cargar=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/soporte?estado=${filtro}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok){
        const d=await r.json();
        // Suena cuando aparece un caso nuevo que necesita a alguien
        if(derivRef.current && (d.derivados||0)>derivRef.current) sonar();
        derivRef.current=d.derivados||0;
  ...
  const abrir=async(id)=>{
    ...
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/soporte/${id}`,
        {headers:authHeaders(agencia.token)});
      if(r.ok){ setDet(await r.json()); cargar(); }
    }catch(e){}
  };
  useEffect(()=>{ finRef.current?.scrollIntoView({behavior:"smooth"}); },[det]);

  const responder=async(cerrar)=>{
    const t=texto.trim();
    if((!t&&!cerrar)||proc) return;
    setProc(true); setTexto("");
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/soporte/${abierto}`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},
        body:JSON.stringify({texto:t,cerrar})});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok){
        if(cerrar){ setAbierto(null); setDet(null); }
        else abrir(abierto);
        cargar();
      }
    }catch(e){}
  ...
  const AUTOR={cliente:"CLIENTE", ia:"ASISTENTE",
               agencia:"VOS", admin:"ADMINISTRACIÓN"};
  ...
  if(abierto&&det){
    return(
      <div style={{display:"flex",flexDirection:"column",
        ...
        <div style={{display:"flex",alignItems:"center",gap:SPACING[12],
          marginBottom:10}}>
          <button onClick={()=>{setAbierto(null);setDet(null);}}
        ...
        <div style={{flex:1,overflowY:"auto",marginBottom:10}}>
          {(det.mensajes||[]).map((m,i)=>(
            <div key={i} style={{display:"flex",
              justifyContent:m.autor==="agencia"?"flex-end":"flex-start",
              marginBottom:8}}>
              <div style={{maxWidth:"85%",
                background:m.autor==="agencia"?`${Q.violet}28`
        ...
        <div style={{display:"flex",gap:SPACING[8],marginBottom:7}}>
          ...
          <button onClick={()=>responder(false)}
            ...
            style={{background:texto.trim()?Q.violet:"rgba(255,255,255,0.05)",
              border:"none",borderRadius:RADII.md,padding:"0 16px",
  ...
  const derivados=(tickets||[]).filter(t=>t.derivado).length;
  ...
  return(
    <div>
      ...
      {!tickets&&<div style={{color:Q.muted,textAlign:"center",
        padding:SPACING[20]}}>Cargando…</div>}
      {tickets&&tickets.length===0&&(
        <GCard style={{padding:SPACING[20],textAlign:"center"}}>
          <div style={{color:Q.muted,fontSize:12.5,
            fontFamily:F_BODY}}>
            No hay consultas en este filtro</div>
        </GCard>
      )}

      {(tickets||[]).map(t=>(
        <GCard key={t.id} glow={t.derivado?Q.gold:null}
          onClick={()=>abrir(t.id)}
              ...
              <div style={{color:Q.text,fontWeight:700,fontSize:12.5,
                ...
                {t.sin_leer>0&&(
                  <span style={{background:Q.red,color:"#fff",borderRadius:RADII.md,
                    padding:"4px 8px",fontSize:12,marginLeft:6}}>
                    {t.sin_leer}</span>
                )}
              </div>
              <div style={{color:Q.muted,fontSize:12,marginTop:2,
...
// ── Mensajes con el admin ─────────────────────────────────────
// Los canales por donde los clientes pueden escribirle a la agencia.
// Aparecen como botones en el chat de soporte del jugador: si están
// vacíos, el cliente no tiene forma de llegar a una persona.
// Qué productos ofrece cada agencia de su red. Puede quitárselos,
// pero no darles algo que ella misma no tenga: si la casa le sacó
// casino a la rama, no puede devolvérselo a nadie.
// Los proveedores de casino de tu red. Podés quitárselos a tus
// subagencias, pero no darles algo que vos no tengas: si la casa lo
// apagó para tu rama, nadie del medio puede devolverlo.
function ProveedoresAgencia({ agencia, onSesionExpirada }){
  ...
  const cargar=()=>{
    fetch(`${API_URL}/api/agencias/me/proveedores`,
      {headers:authHeaders(agencia.token)})
      .then(r=>{ if(r.status===401){onSesionExpirada();return null;}
        return r.ok?r.json():null; })
      .then(x=>x&&setD(x)).catch(()=>{});
  };
  useEffect(cargar,[]);

  const cambiar=async(code,marca,activo)=>{
    let motivo=null;
    if(!activo){
  ...
  if(!d) return null;
  if(!(d.hijas||[]).length) return null;

  const lista=(d.proveedores||[]).filter(p=>{
    if(filtro==="vivo") return p.es_vivo;
    if(filtro==="slots") return !p.es_vivo;
    return true;
  });
    ...
    <div style={{marginTop:16,paddingTop:SPACING[16],
      ...
      {d.hijas.map(h=>(
        <div key={h.code} style={{marginBottom:7}}>
          <div onClick={()=>setAbierta(abierta===h.code?null:h.code)}
            style={{display:"flex",justifyContent:"space-between",
            ...
            <div style={{minWidth:0,flex:1}}>
              ...
              <div style={{color:Q.dim,fontSize:12}}>
                ...
                {h.apagados_por_mi.length>0&&(
                  <span style={{color:Q.amber}}>
                    {" "}· {h.apagados_por_mi.length} quitado
          ...
          {abierta===h.code&&(
            <div style={{paddingLeft:SPACING[8],paddingBottom:SPACING[8]}}>
              ...
              {lista.map(p=>{
                const tiene=!h.apagados.includes(p.marca);
                const puedoTocar=p.lo_tengo&&(
                  !tiene ? h.apagados_por_mi.includes(p.marca) : true);
                return(
                  <button key={p.marca}
                    onClick={()=>puedoTocar&&cambiar(h.code,p.marca,!tiene)}
                    disabled={proc||!puedoTocar}
                    style={{width:"100%",display:"flex",
                      justifyContent:"space-between",alignItems:"center",
                      background:!p.lo_tengo?"rgba(255,255,255,0.02)"
...
function ProductosRed({ agencia, onSesionExpirada }){
  ...
  if(!datos.hijas?.length) return null;
    ...
    <div style={{marginBottom:12,paddingBottom:SPACING[12],
      ...
      {datos.hijas.map(h=>(
        <div key={h.code} style={{marginBottom:7}}>
          ...
          {abierta===h.code&&(
            <div style={{paddingLeft:SPACING[12],paddingBottom:SPACING[8]}}>
              {datos.catalogo.map(c=>{
                const loTengo=(datos.mis_productos||[]).includes(c.codigo);
                const loTiene=h.productos.includes(c.codigo);
                // Si yo no lo tengo, no puedo dárselo: el botón queda
                // inerte y se explica por qué.
                return(
                  <button key={c.codigo}
                    onClick={()=>loTengo&&cambiar(h.code,c.codigo,!loTiene)}
                    disabled={proc||!loTengo}
                    style={{width:"100%",display:"flex",
                      justifyContent:"space-between",alignItems:"center",
                      background:!loTengo?"rgba(255,255,255,0.02)"
...
// Las pantallas del local. Cada una tiene su QR: el cliente lo
// escanea, arma el boleto en su teléfono mientras espera, y llega al
// mostrador con el código listo. El cajero solo cobra.
function Terminales({ agencia, onSesionExpirada }){
  ...
  const cargar=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/terminales`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok) setLista((await r.json()).terminales||[]);
    }catch(e){ setLista([]); }
  };
  useEffect(()=>{ cargar(); /* eslint-disable-next-line */ },[]);
  ...
  const cambiar=async(t,campo,valor)=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/terminales/${t.id}`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},
    ...
    <div>
      ...
      <GCard style={{padding:SPACING[16],marginBottom:12}}>
        <div style={{color:Q.text,fontWeight:700,fontSize:12.5,
          ...
          Nueva terminal</div>
        <input value={nombre} onChange={e=>setNombre(e.target.value)}
          placeholder="Nombre: Terminal 1, Barra, Entrada…" style={inp}/>
        <input value={ubic} onChange={e=>setUbic(e.target.value)}
          placeholder="Dónde está (opcional)" style={inp}/>
        <Btn label={proc?"Creando…":"+ Crear terminal"} onClick={crear}
          color={Q.violet} full disabled={proc}/>
      </GCard>

      {!lista&&<div style={{color:Q.muted,textAlign:"center",
        padding:SPACING[20]}}>Cargando…</div>}
      ...
      {lista&&lista.length===0&&(
        <GCard style={{padding:SPACING[24],textAlign:"center"}}>
          <div style={{color:Q.muted,fontSize:13,lineHeight:1.55,
            fontFamily:F_BODY}}>
            Todavía no tenés terminales.<br/>
            Creá una por cada pantalla del local.</div>
        </GCard>
      )}

      {(lista||[]).map(t=>(
        <GCard key={t.id} glow={t.activa?null:Q.red}
          style={{padding:SPACING[12],marginBottom:8,opacity:t.activa?1:0.6}}>
          <div style={{display:"flex",justifyContent:"space-between",
            alignItems:"flex-start",gap:SPACING[8]}}>
            <div style={{minWidth:0,flex:1}}>
              <div style={{color:Q.text,fontWeight:700,fontSize:13,
            ...
            <button onClick={()=>setVerQR(verQR===t.codigo?null:t.codigo)}
              style={{background:`${Q.cyan}18`,
                ...
                padding:"8px 12px",cursor:"pointer",color:Q.cyan,
          ...
          {verQR===t.codigo&&(
            <div style={{marginTop:11,paddingTop:SPACING[12],
              borderTop:`1px solid ${Q.border}`,textAlign:"center"}}>
              {/* El QR se genera al vuelo: no hace falta guardar una
              ...
              <div style={{display:"flex",gap:SPACING[8],marginTop:11}}>
...
// Los desafíos de la red: cuánto se juega, qué comisión deja, y qué
// subagencias lo tienen habilitado.
//
// Una agencia puede QUITARLE el producto a sus subagencias, pero no
// dárselo si ella misma no lo tiene: si la casa se lo sacó a la
// rama, nadie del medio puede devolverlo.
// Le explica al agente cómo viene su negocio con sus propios
// números. No inventa datos ni promete resultados: un agente que
// decide sobre una proyección optimista termina peor que sin asesor.
function AsesorAgencia({ agencia, onSesionExpirada }){
  const [d,setD]=useState(null);
  const [cargando,setCargando]=useState(false);
  ...
  const analizar=async()=>{
    ...
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/asesor`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
  ...
  useEffect(()=>{ analizar(); /* eslint-disable-next-line */ },[]);

  const preguntar=async()=>{
    if(pregunta.trim().length<5) return;
    setProc(true); setRespuesta("");
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/asesor/preguntar`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},
    ...
    }catch(e){ setRespuesta("⚠️ "+e.message); }
    setProc(false);
  };

  const SUGERIDAS=[
    "¿Qué hago con los clientes que dejaron de jugar?",
    "¿Cómo vengo comparado con el mes pasado?",
    "¿Me conviene abrir otra subagencia?",
  ];
  ...
  return(
    <div>
      ...
      {d&&(
        <>
          <GCard glow={Q.cyan} style={{padding:SPACING[16],margin:"12px 0"}}>
            ...
            <div style={{display:"flex",gap:SPACING[12],paddingTop:SPACING[8],
              borderTop:`1px solid ${Q.border}`}}>
              <div style={{flex:1}}>
          ...
          {d.analisis&&(
            <GCard style={{padding:SPACING[16],marginBottom:12}}>
              <div style={{color:Q.text,fontSize:13,lineHeight:1.65,
                whiteSpace:"pre-wrap",
                fontFamily:F_BODY}}>
                {d.analisis}</div>
            </GCard>
      ...
      <input value={pregunta} onChange={e=>setPregunta(e.target.value)}
        ...
        style={{width:"100%",background:"rgba(255,255,255,0.05)",
          border:`1px solid ${Q.border}`,borderRadius:RADII.md,
          padding:"12px 12px",color:Q.text,fontSize:13.5,marginBottom:8,
          fontFamily:F_BODY}}/>

      <div style={{display:"flex",gap:SPACING[4],flexWrap:"wrap",marginBottom:10}}>
        {SUGERIDAS.map(q=>(
      ...
      <Btn label={proc?"Pensando…":"Preguntar"} onClick={preguntar}
        color={Q.cyan} full disabled={proc||pregunta.trim().length<5}/>

      {respuesta&&(
        <GCard glow={Q.cyan} style={{padding:SPACING[16],marginTop:12}}>
          <div style={{color:Q.text,fontSize:13,lineHeight:1.65,
            whiteSpace:"pre-wrap",
            fontFamily:F_BODY}}>{respuesta}</div>
        </GCard>
      )}
    </div>
  );
...
function DesafiosAgencia({ agencia, onSesionExpirada }){
  ...
  const cargarLiq=async()=>{
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/liquidacion?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
  ...
  const cargarCC=async()=>{
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/desafios/cuenta?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
      if(r.ok) setCc(await r.json());
    }catch(e){}
  };
  ...
  const cargarReporte=async()=>{
    ...
    try{
      const r=await fetch(
        `${API_URL}/api/agencias/me/desafios/reporte?desde=${desde}&hasta=${hasta}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
  ...
  const cargarProds=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/productos`,
        {headers:authHeaders(agencia.token)});
      if(r.ok) setProds(await r.json());
    }catch(e){}
  };

  useEffect(()=>{ cargarReporte(); cargarProds(); cargarCC(); cargarLiq();
    /* eslint-disable-next-line */ },[]);
  ...
  const cambiar=async(code,activo)=>{
    ...
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/productos`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},
        body:JSON.stringify({agencia_code:code, producto:"desafios",
                             activo})});
      const d=await r.json();
      if(!r.ok) throw new Error(d.detail||`Error ${r.status}`);
  ...
  return(
    <div>
      ...
      <Btn label={cargando?"Cargando…":"Ver"} onClick={cargarReporte}
        color={Q.violet} full disabled={cargando}/>

      {/* El número final: qué se le paga cruzando todo. Es lo
          primero que quiere ver el agente. */}
      {liq&&(
        <GCard glow={liq.total_liquidar>=0?Q.green:Q.amber}
          style={{padding:SPACING[16],margin:"12px 0"}}>
          <div style={{color:Q.muted,fontSize:12,letterSpacing:0.8,
          ...
          <div style={{display:"flex",justifyContent:"space-between",
            ...
            <span style={{color:Q.text,fontSize:13,fontWeight:700,
              fontFamily:F_BODY}}>
              {liq.total_liquidar>=0?"Te pagamos":"Le debés a la casa"}</span>
            <span style={{color:liq.total_liquidar>=0?Q.green:Q.amber,
              fontWeight:900,fontSize:22,
              fontFamily:F_BODY}}>
              {ars(Math.abs(liq.total_liquidar))}</span>
          </div>
          {[["Deportivas",liq.deportivas?.comision||0],
            ["Casino",liq.casino?.comision||0],
            ["Desafíos",liq.desafios?.comision||0],
            ["Movimiento de desafíos",liq.desafios?.saldo_movimiento||0],
      ...
      {cc&&(cc.desafios>0||cc.saldo!==0)&&(
        <GCard glow={cc.saldo>=0?Q.green:Q.amber}
          ...
          <div style={{display:"flex",justifyContent:"space-between",
            ...
            <span style={{color:Q.text,fontSize:13,fontWeight:700,
              fontFamily:F_BODY}}>
              {cc.saldo>=0?"Te transferimos":"Le debés al sistema"}</span>
            <span style={{color:cc.saldo>=0?Q.green:Q.amber,
          ...
          <div style={{color:Q.dim,fontSize:12,marginTop:7,
            ...
            tus jugadores
            {cc.premios_a_pagar>0&&
              `, tenés que pagar ${cc.premios_a_pagar.toLocaleString("es-AR")}`}
            . Ganaste {cc.rake.toLocaleString("es-AR",
        ...
        <GCard glow={Q.cyan} style={{padding:SPACING[16],margin:"12px 0"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",
            ...
            <span style={{color:Q.green,fontWeight:900,fontSize:20,
              fontFamily:F_BODY}}>
              {rep.mi_comision.toLocaleString("es-AR",
                {maximumFractionDigits:2})}</span>
          </div>
          {rep.comision_generada>rep.mi_comision&&(
            <div style={{color:Q.dim,fontSize:12,marginTop:6,
              ...
              subagencias y a la casa.</div>
          )}
        </GCard>
      )}

      {/* Quitárselo o dárselo a las subagencias */}
      {prods&&(prods.hijas||[]).length>0&&(
        <GCard style={{padding:SPACING[16],marginTop:12}}>
          ...
          <div style={{color:Q.dim,fontSize:12,marginBottom:10,
            ...
            {loTengo
              ...
              :"No podés habilitarlo porque vos no lo tenés."}</div>

          {prods.hijas.map(h=>{
            const tiene=(h.productos||[]).includes("desafios");
            return(
              <button key={h.code}
                onClick={()=>loTengo&&cambiar(h.code,!tiene)}
                disabled={proc||!loTengo}
                style={{width:"100%",display:"flex",
                  justifyContent:"space-between",alignItems:"center",
                  background:!loTengo?"rgba(255,255,255,0.02)"
...
function MisCanales({ agencia, onSesionExpirada }){
  ...
  useEffect(()=>{
    fetch(`${API_URL}/api/agencias/me/contacto`,
      {headers:authHeaders(agencia.token)})
      .then(r=>r.ok?r.json():null)
      .then(d=>{ if(d){ setWa(d.whatsapp||""); setTg(d.telegram_url||"");
                        setHor(d.horario||""); } })
      .catch(()=>{});
    // eslint-disable-next-line
  },[]);
  ...
  const guardar=async()=>{
    ...
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/contacto`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},
        body:JSON.stringify({whatsapp:wa,telegram_url:tg,horario:hor})});
      if(r.status===401){ onSesionExpirada(); return; }
      const d=await r.json();
      if(!r.ok) throw new Error(d.detail||`Error ${r.status}`);
      ...
      {abierto&&(
        <div style={{marginTop:9}}>
          ...
          <input value={wa} onChange={e=>setWa(e.target.value)}
            placeholder="WhatsApp: +54 9 11 1234 5678" style={inp}/>
          <input value={tg} onChange={e=>setTg(e.target.value)}
            placeholder="Telegram: @tuusuario" style={inp}/>
...
function MensajesAgencia({ agencia, onSesionExpirada }){
  ...
  const cargar=async()=>{
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/mensajes`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok) setMsgs((await r.json()).mensajes||[]);
    }catch(e){ setMsgs([]); }
  };
  useEffect(()=>{
    cargar();
  ...
  const enviar=async()=>{
    ...
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/mensajes`,{
        method:"POST",headers:{"Content-Type":"application/json",
          ...authHeaders(agencia.token)},body:JSON.stringify({texto:t})});
      if(r.status===401){ onSesionExpirada(); return; }
      if(r.ok) cargar();
    }catch(e){}
    setProc(false);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",
      ...
      {/* Los canales que ven los clientes en su chat de soporte */}
      <MisCanales agencia={agencia} onSesionExpirada={onSesionExpirada}/>
      <ProductosRed agencia={agencia} onSesionExpirada={onSesionExpirada}/>
      <ProveedoresAgencia agencia={agencia} onSesionExpirada={onSesionExpirada}/>
...
function SaldoCC({ agencia, onSesionExpirada }){
  ...
  if(!data) return <div style={{color:Q.muted,padding:SPACING[20],textAlign:"center",
    fontFamily:F_BODY}}>Cargando saldo...</div>;

  return(
    <div>
      <GCard glow={data.saldo_cc>=0?Q.green:Q.red} style={{padding:SPACING[20],marginBottom:14,
      ...
      {data.movimientos.length===0&&(
        <GCard style={{padding:SPACING[20],textAlign:"center"}}>
          <div style={{color:Q.muted,fontSize:12,
            fontFamily:F_BODY}}>Sin movimientos todavía</div>
        </GCard>
      )}
...
function PSPAgencia({ agencia, onSesionExpirada }){
  const [tipo,setTipo]=useState("cargas");
  const [items,setItems]=useState(null);
  const [err,setErr]=useState("");
  const cargar=async()=>{
    setItems(null); setErr("");
    try{
      const r=await fetch(`${API_URL}/api/agencias/me/psp/historial?tipo=${tipo}`,
        { headers: authHeaders(agencia.token) });
      if(r.status===401){ onSesionExpirada(); return; }
      if(!r.ok) throw new Error(`Error ${r.status}`);
      setItems((await r.json()).items||[]);
    }catch(e){ setErr(e.message==="Failed to fetch"?"Sin conexión":e.message); setItems([]); }
  };
  useEffect(()=>{ cargar(); /* eslint-disable-next-line */ },[tipo]);

  const lista=items||[];
  const total=lista.filter(x=>["acreditado","completado"].includes(x.estado))
    .reduce((a,x)=>a+(x.monto||0),0);
  const colores={acreditado:Q.green,completado:Q.green,pendiente:Q.amber,
    solicitado:Q.amber,procesando:Q.cyan,vencido:Q.muted,rechazado:Q.red,fallido:Q.red};
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:SPACING[8],marginBottom:12}}>
        {[["cargas",<><Icon name="arrow-up-right" size={12}/> Cargas</>],["retiros",<><Icon name="arrow-down-left" size={12}/> Retiros</>]].map(([k,l])=>(
          <button key={k} onClick={()=>setTipo(k)} style={{flex:1,
            ...
            padding:"8px",cursor:"pointer",color:tipo===k?Q.violet2||Q.violet:Q.muted,
      ...
      <GCard glow={Q.green} style={{padding:"12px 16px",marginBottom:12,textAlign:"center"}}>
        <div style={{color:Q.muted,fontSize:12,textTransform:"uppercase",letterSpacing:1,
          fontFamily:F_BODY}}>
          Total {tipo==="cargas"?"acreditado":"completado"}</div>
        <div style={{color:Q.green,fontWeight:800,fontSize:20,
          fontFamily:F_BODY}}>{ars(total)}</div>
      </GCard>

      {items===null&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[16],
        fontFamily:F_BODY}}>Cargando...</div>}
      {items&&lista.length===0&&<GCard style={{padding:SPACING[24],textAlign:"center"}}>
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:SPACING[8],marginBottom:14}}>
        {[["Otorgados",res.otorgados||0,Q.cyan],["Activos",res.activos||0,Q.gold],
          ...
          <GCard key={i} glow={c} style={{padding:"12px 8px",textAlign:"center"}}>
      ...
      {bonos.map(b=>(
        <GCard key={b.id} glow={Q.gold} style={{padding:SPACING[16],marginBottom:8}}>
...
function HistorialCashout({ agencia, onSesionExpirada }){
  ...
  return(
    <div>
      ...
      {cargando&&<GCard style={{padding:SPACING[20],textAlign:"center"}}>
...
function Historial({ agencia, onSesionExpirada }){
  ...
  const [err,setErr]=useState("");
  const [cargando,setCargando]=useState(true);
  // El permiso de anular lo habilita el admin, por agencia o por rama.
  const [anular,setAnular]=useState({puede_anular:false});

  useEffect(()=>{
    fetch(`${API_URL}/api/agencias/me/puede-anular`,
      {headers:authHeaders(agencia.token)})
      .then(r=>r.ok?r.json():null)
      .then(d=>d&&setAnular(d))
      .catch(()=>{});
  },[agencia.token]);
...
function Cierres({ agencia, onSesionExpirada }){
  ...
  // Caja: cargas, descargas y saldo vivo. Es lo que el cajero necesita
  // para cuadrar al cierre del día.
  ...
  const cargarCaja=async()=>{
    ...
    try{
      ...
      if(filtroCli) q.set("cliente",filtroCli);
      const r=await fetch(`${API_URL}/api/agencias/me/caja?${q}`,
        {headers:authHeaders(agencia.token)});
      if(r.status===401){ onSesionExpirada(); return; }
      if(!r.ok){
        const e=await r.json().catch(()=>({}));
    ...
    <div>
      ...
      {vista==="caja"&&(
        <div>
            ...
            <GCard glow={Q.cyan} style={{padding:SPACING[16],marginBottom:12}}>
              ...
              <div style={{color:Q.muted,fontSize:12,marginBottom:10}}>
                ...
                {caja.cliente.saldo_bono>0&&
                  <span style={{color:Q.gold}}> · bono {ars(caja.cliente.saldo_bono)}</span>}
              </div>
          ...
          {caja&&!caja.error&&!caja.cliente&&(
            <>
              {/* El efectivo que pasó por tu mostrador: lo que
                  recibiste por ventas y lo que pagaste en premios.
...
{/* El desglose: el cajero tiene que poder cuadrar
              ...
              {(caja.agencias||[]).map(a=>(
                <GCard key={a.code} glow={a.es_mia?Q.cyan:null}
          ...
          <GCard glow={Q.green} style={{padding:SPACING[16],marginBottom:12,
            ...
            {!res.es_cliente&&res.total.neto_sin_influencers!=null&&(
              <div style={{marginTop:12,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{color:Q.text,fontWeight:700,fontSize:13,
                    fontFamily:F_BODY}}>Neto (sin influencers)</span>
                  <span style={{color:res.total.neto_sin_influencers>=0?Q.green:Q.red,
                    fontWeight:800,fontSize:18,
                    fontFamily:F_BODY}}>{ars(res.total.neto_sin_influencers)}</span>
                </div>
                {res.total.comision_influencers>0&&(
                  <div style={{display:"flex",justifyContent:"space-between",
                    ...
                    <span style={{color:Q.gold,fontWeight:700,fontSize:13,
                      fontFamily:F_BODY}}>−{ars(res.total.comision_influencers)}</span>
                  </div>
                )}
                <div style={{display:"flex",justifyContent:"space-between",
                  alignItems:"center",marginTop:8,paddingTop:SPACING[8],
                  borderTop:`1px solid ${Q.dim}`}}>
                  <span style={{color:Q.text,fontWeight:700,fontSize:13,
                    fontFamily:F_BODY}}>Neto final</span>
                  <span style={{color:res.total.neto_con_influencers>=0?Q.green:Q.red,
                    fontWeight:900,fontSize:20,
                    fontFamily:F_BODY}}>{ars(res.total.neto_con_influencers)}</span>
                </div>
              </div>
            )}
          </GCard>

          {/* Casino y ruleta de tu red: se liquidan aparte, con su
              propia comisión sobre GGR. */}
          {res.casino&&(res.casino.apostado>0||res.casino.ggr!==0)&&(
            <GCard glow={Q.gold} style={{padding:SPACING[16],marginBottom:12}}>
              ...
              {[["Apostado",res.casino.apostado,Q.text],
                ...
                ["Tu comisión",res.casino.comisiones,Q.cyan]].map(([l,v,c])=>(
                <div key={l} style={{display:"flex",
          ...
          {res.total_general&&(
            <GCard glow={Q.cyan} style={{padding:SPACING[16],marginBottom:12}}>
              ...
              <div style={{display:"flex",gap:SPACING[12],marginBottom:10}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{color:res.total_general.ggr>=0?Q.green:Q.red,
                    fontWeight:800,fontSize:16,
                    fontFamily:F_BODY}}>
                    {ars(res.total_general.ggr)}</div>
                </div>
              </div>
              {/* Lo que vos pusiste en bonos: resta de tu comisión */}
              {res.total_general.bonos>0&&(
                <div style={{display:"flex",justifyContent:"space-between",
                  fontSize:12,padding:"4px 0",marginBottom:4,
                  paddingBottom:SPACING[8],borderBottom:`1px solid ${Q.border}`}}>
                  <span style={{color:Q.muted}}>Bono sports tu parte</span>
                  <span style={{color:Q.red,
                    fontFamily:F_BODY}}>
                    −{ars(res.total_general.bonos)}</span>
                </div>
              )}
              ...
              <div style={{display:"flex",justifyContent:"space-between",
                ...
                <span style={{color:Q.text,fontWeight:700,fontSize:13,
                  fontFamily:F_BODY}}>
                  Tu comisión total</span>
                <span style={{color:Q.green,fontWeight:900,fontSize:22,
                  fontFamily:F_BODY}}>
                  {ars(res.total_general.mi_comision)}</span>
              </div>
            </GCard>
          )}
          ...
          {res.agencias.map(a=>(
            <GCard key={a.code} glow={a.es_mia?Q.violet:undefined}
              ...
              <div style={{display:"flex",justifyContent:"space-between",
                ...
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  fontFamily:F_BODY}}>
                  {a.es_mia?<><Star size={12}/> </>:""}{a.name}</div>
                <div style={{color:Q.gold,fontWeight:700,fontSize:13,
      ...
      {vista==="apuestas"&&<>
        {apuestas===null&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
          fontFamily:F_BODY}}>Cargando...</div>}
      ...
      {vista==="impresiones"&&<>
        {impresiones===null&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
          fontFamily:F_BODY}}>Cargando...</div>}
        {impresiones&&impresiones.length===0&&<div style={{color:Q.muted,fontSize:12,textAlign:"center",
          padding:SPACING[20],fontFamily:F_BODY}}>Sin impresiones en el período</div>}
...
function ConectarTelegramAgencia({ agencia }){
  ...
  return(
    <GCard glow={Q.cyan} style={{padding:SPACING[20],marginBottom:12}}>
      <div style={{color:Q.text,fontWeight:700,fontSize:14,marginBottom:6,
        fontFamily:F_BODY}}><Smartphone size={14}/> Conectar Telegram</div>
      ...
      {link&&(
        <div style={{marginTop:10,display:"flex",gap:SPACING[8]}}>
...
function CambiarMiPassword({ agencia, obligado=false, onListo }){
  ...
  return(
    <GCard glow={obligado?Q.amber:Q.violet} style={{padding:SPACING[20],marginBottom:12}}>
      <div style={{color:Q.text,fontWeight:700,fontSize:14,marginBottom:8,
        fontFamily:F_BODY}}>
        {obligado?<><Key size={14}/> Cambiá tu contraseña para continuar</>:<><Key size={14}/> Cambiar mi contraseña</>}</div>
...
function EnVivo({ agencia }){
  ...
  // Código de influencer, opcional. Estaba en el cuerpo del pedido
  // pero sin declarar: al enviar la apuesta en vivo reventaba.
  const [codigoInf,setCodigoInf]=useState("");
  ...
  const resetLive=()=>{ setTicket([]); setSlip(null); setCliente("");
                        setMonto(montoInicial(agencia?.moneda)); setErrGuardar(""); };
  ...
  return(
    <div>
      ...
      {loading&&(
        <GCard style={{padding:SPACING[24],textAlign:"center"}}>
          <div style={{color:Q.muted,fontFamily:F_BODY}}>Cargando partidos en vivo...</div>
        </GCard>
      )}
      ...
      {matches
        ...
        .map(m=>(
        <GCard key={m.id} glow={Q.pink} style={{padding:"16px 16px",marginBottom:10,
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{flex:1,display:"flex",alignItems:"center",gap:SPACING[8],minWidth:0}}>
            ...
            <div style={{textAlign:"center",padding:"0 12px"}}>
              <div style={{fontFamily:F_BODY,fontWeight:900,
                fontSize:26,color:Q.pink}}>
                {m.homeScore}<span style={{color:Q.dim}}> - </span>{m.awayScore}
              </div>
            </div>
            <div style={{flex:1,display:"flex",alignItems:"center",gap:SPACING[8],
      ...
      {ticket.length>0&&(
        <div style={{position:"sticky",bottom:0,zIndex:20,
          ...
          <GCard glow={Q.pink} style={{padding:"12px 16px"}}>
          ...
          <input type="number" inputMode="numeric" min="0" value={monto}
            ...
            <div style={{display:"flex",gap:SPACING[8]}}>
              <button onClick={()=>setTicket([])} disabled={guardando} style={{
                background:"transparent",border:`1px solid ${Q.border}`,
                borderRadius:RADII.lg,padding:"0 16px",cursor:"pointer",color:Q.muted,
                fontSize:12,fontFamily:F_BODY,
              }}>Limpiar</button>
...
function InfluencersAgencia({ agencia, onSesionExpirada }){
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:SPACING[8],marginBottom:12}}>
        {[["mios",<><Star size={13}/> Míos</>],["costos",<><Banknote size={13}/> Cuestan</>],["escaneos",<><Icon name="scan-line" size={13}/> Escaneos</>]].map(([k,l])=>(
          <button key={k} onClick={()=>setVista(k)} style={{flex:1,
            background:vista===k?`${Q.violet}33`:"rgba(255,255,255,0.04)",
            border:`1px solid ${vista===k?Q.violet:Q.border}`,borderRadius:RADII.md,
            padding:"8px",cursor:"pointer",color:vista===k?Q.cyan:Q.muted,
      ...
      {vista==="costos"&&<>
        {!costos&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
          fontFamily:F_BODY}}>Cargando...</div>}
        {costos&&(
          <div>
            <GCard glow={Q.gold} style={{padding:SPACING[16],marginBottom:12,textAlign:"center"}}>
              <div style={{color:Q.muted,fontSize:12,textTransform:"uppercase",letterSpacing:1,
                marginBottom:4,fontFamily:F_BODY}}>
      ...
      {vista==="escaneos"&&<>
        {!escaneos&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
          fontFamily:F_BODY}}>Cargando...</div>}
        {escaneos&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:SPACING[8],marginBottom:12}}>
    ...
    <GCard glow={Q.violet} style={{padding:SPACING[16],marginBottom:14}}>
      ...
      {[["name","Nombre"],["username","Usuario"],["password","Contraseña (8+)"]].map(([k,l])=>(
        <input key={k} value={form[k]} type={k==="password"?"password":"text"}
          ...
          style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
            borderRadius:RADII.md,padding:"12px 12px",color:Q.text,fontSize:16,marginBottom:8,
            fontFamily:F_BODY}}/>
      ))}
      <div style={{display:"flex",gap:SPACING[8],marginBottom:8}}>
        <input value={form.pct_ggr} onChange={e=>setForm(f=>({...f,pct_ggr:e.target.value}))}
          ...
          style={{flex:1,background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
            borderRadius:RADII.md,padding:"12px 12px",color:Q.text,fontSize:16,
            fontFamily:F_BODY}}/>
      ...
      <select value={form.alcance} onChange={e=>setForm(f=>({...f,alcance:e.target.value}))}
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
          borderRadius:RADII.md,padding:"12px 12px",color:form.alcance?Q.text:Q.muted,fontSize:15,marginBottom:10,
          fontFamily:F_BODY}}>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:210,
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:Q.deep,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{color:Q.text,fontWeight:700,fontSize:16,
            fontFamily:F_BODY}}><Star size={15}/> {rep?rep.name:code}</div>
          <button onClick={onCerrar} style={{background:"transparent",border:"none",
            color:Q.muted,fontSize:26,cursor:"pointer",lineHeight:1}}>×</button>
        </div>
...
function ConfigurarCuentaAg({ agencia, cuenta, onCambio, onSesionExpirada }){
  ...
  // Casino y desafíos se liquidan aparte del deportivo. Una agencia
  // puede repartir a sus subagencias solo lo que ella cobra.
  ...
  const guardar=async()=>{
    ...
    try{
      const body={name,address,phone,
        ...
        pct_ggr_casino:parseFloat(ggrCasino)||0,
        pct_desafios:parseFloat(pctDesafios)||0};
  ...
  return(
    <div>
      ...
      {/* Acá había un selector de permisos que usaba un objeto "form"
          inexistente en este componente: la pantalla reventaba con
...
function MisAgencias({ agencia, onSesionExpirada }){
  ...
  return(
    <div>
      ...
      {!arbol&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
        fontFamily:F_BODY}}>Cargando...</div>}
      ...
      {arbol&&arbol.agencias.map(a=>{
        // El backend guarda 'suspended', no 'bloqueada' ni 'blocked'.
        // Con la comparación vieja esto era SIEMPRE falso: el botón
        // ofrecía "Bloquear" incluso con la agencia ya bloqueada, y no
        // había forma de desbloquearla. Se compara contra 'active', que
        // además aguanta cualquier estado nuevo.
        const bloqueada = !!a.status && a.status !== "active";
        return(
          <GCard key={a.code} style={{padding:"12px 16px",marginBottom:8,
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:SPACING[8]}}>
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  ...
                  {bloqueada&&<span style={{color:Q.red,fontSize:12}}> <Lock size={11}/></span>}</div>
                ...
                {a.parent_code&&<div style={{color:Q.violet2,fontSize:12,marginTop:1}}>
                  <Building2 size={11}/> {a.parent_code===agencia.code
                    ? agencia.name
              ...
              <div style={{display:"flex",flexDirection:"column",gap:SPACING[4],flexShrink:0}}>
                <button onClick={()=>setCcPara(a.code)} style={{background:`${Q.cyan}18`,
                  border:`1px solid ${Q.cyan}`,borderRadius:RADII.sm,padding:"4px 12px",cursor:"pointer",
                ...
                <button onClick={()=>bloquear(a.code,!bloqueada)} style={{background:"transparent",
                  border:`1px solid ${bloqueada?Q.green:Q.red}`,borderRadius:RADII.sm,padding:"4px 12px",
                ...
                <button onClick={()=>setResetPara({code:a.code,nombre:a.name})} style={{background:`${Q.amber}18`,
                  border:`1px solid ${Q.amber}`,borderRadius:RADII.sm,padding:"4px 12px",cursor:"pointer",
                  color:Q.amber,fontSize:12,fontWeight:700,
                  fontFamily:F_BODY}}><Key size={11}/> Clave</button>
                <button onClick={()=>setConfigPara(a)} style={{background:`${Q.violet}18`,
                  border:`1px solid ${Q.violet}`,borderRadius:RADII.sm,padding:"4px 12px",cursor:"pointer",
      ...
      {configPara&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:220,
          ...
          <GCard glow={Q.violet} onClick={e=>e.stopPropagation()}
            style={{padding:SPACING[20],maxWidth:420,width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{color:Q.text,fontWeight:700,fontSize:15,
...
function CrearSubAgencia({ agencia, onListo, onSesionExpirada }){
  ...
  return(
    <GCard glow={Q.violet} style={{padding:SPACING[16],marginBottom:14}}>
      <div style={{color:Q.violet2,fontWeight:700,fontSize:14,marginBottom:12,
        fontFamily:F_BODY}}><Building2 size={14}/> Nueva sub-agencia</div>
      {[["name","Nombre"],["username","Usuario"],["password","Contraseña (8+)"]].map(([k,l])=>(
        <input key={k} value={form[k]} type={k==="password"?"password":"text"}
          onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} placeholder={l}
          style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
      ...
      <select value={form.permiso} onChange={e=>setForm(f=>({...f,permiso:e.target.value}))}
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.violet}`,
          borderRadius:RADII.md,padding:"12px 12px",color:Q.text,fontSize:15,marginBottom:8,
          fontFamily:F_BODY}}>
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:210,
      ...
      <GCard glow={Q.cyan} onClick={e=>e.stopPropagation()}
        style={{padding:SPACING[20],maxWidth:380,width:"100%"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{color:Q.text,fontWeight:700,fontSize:15,
        ...
        <div style={{display:"flex",gap:SPACING[8],marginBottom:12}}>
          {[["cargar",<><Icon name="arrow-up-right" size={12}/> Cargar</>],["retirar",<><Icon name="arrow-down-left" size={12}/> Retirar</>]].map(([k,l])=>(
            <button key={k} onClick={()=>setModo(k)} style={{flex:1,
              ...
              padding:"8px",cursor:"pointer",color:modo===k?Q.cyan:Q.muted,
              fontSize:13,fontWeight:700,fontFamily:F_BODY}}>{l}</button>
          ))}
        </div>
        ...
        <input value={monto} onChange={e=>setMonto(e.target.value)} inputMode="decimal"
          ...
          style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
            borderRadius:RADII.md,padding:"12px 16px",color:Q.text,fontSize:18,marginBottom:12,
...
function AgenciaPanel({ agencia, onLogout, onSesionExpirada }){
  ...
  useEffect(()=>{
    if(!agencia?.token) return;
    let vivo=true;
    const traer=()=>fetch(`${API_URL}/api/agencias/me/mensajes/pendientes`,
        {headers:authHeaders(agencia.token)})
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!vivo||!d) return;
        setMsgPendientes(prev=>{
          const n=d.sin_leer||0;
          if(n>prev) sonarAlerta(false);
          return n;
        });
      }).catch(()=>{});
    traer();
    ...
    <div style={isDesktop ? {
      ...
      <div style={{padding:"16px 12px",maxWidth:620,margin:"0 auto",
        ...
        {/* Los avisos del admin: arriba de todo y en cualquier
            pestaña, para que no dependan de que entren a mirarlos. */}
        <CarruselAvisos agencia={agencia} onSesionExpirada={onSesionExpirada}/>

        {tab==="codigo"   &&<FlujoCodigo  agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        {tab==="envivo"   &&<EnVivo agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        ...
        {tab==="mensajes" &&<MensajesAgencia agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        {tab==="terminales"&&<Terminales agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        {tab==="desafios"&&<DesafiosAgencia agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        {tab==="asesor"&&<AsesorAgencia agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
        {tab==="soporte"  &&<SoporteAgencia agencia={agencia} onSesionExpirada={onSesionExpirada}/>}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:300,
      ...
      <div style={{maxWidth:520,margin:"0 auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          marginBottom:12}}>
          <div style={{color:Q.text,fontWeight:700,fontSize:16,
            fontFamily:F_BODY}}><Palette size={16}/> Editor de placa</div>
          <button onClick={onCerrar} style={{background:"transparent",border:"none",
        ...
        {capaSel&&(
          <GCard style={{padding:SPACING[12],marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:SPACING[8],alignItems:"center",marginBottom:8}}>
              {colores.map(col=>(
            ...
            <div style={{display:"flex",gap:SPACING[8]}}>
              ...
              <button onClick={borrarSel}
                style={{flex:1,background:`${Q.red}22`,border:`1px solid ${Q.red}`,
                  borderRadius:RADII.md,padding:"8px",color:Q.red,cursor:"pointer",fontSize:13}}><Trash2 size={13} role="img" aria-label="Borrar"/></button>
            </div>
          </GCard>
        )}
        ...
        <div style={{display:"flex",gap:SPACING[8],marginBottom:8,flexWrap:"wrap"}}>
          {templates.map((t,i)=>(
            <button key={i} onClick={()=>setFondo({tipo:"template",valor:i})}
              style={{padding:"8px 12px",borderRadius:RADII.md,fontSize:12,cursor:"pointer",
        ...
        <div style={{display:"flex",gap:SPACING[8],marginBottom:10,flexWrap:"wrap"}}>
          {solidos.map((col,i)=>(
        ...
        <div style={{display:"flex",gap:SPACING[4],marginBottom:12,flexWrap:"wrap"}}>
          {emojis.map(em=>(
        ...
        <div style={{display:"flex",gap:SPACING[8]}}>
          <Btn label={<><Smartphone size={13}/> Compartir</>} onClick={compartir} color={Q.violet} full/>
      ...
      <GCard glow={Q.cyan} style={{padding:SPACING[16],marginBottom:12,
        ...
        <div style={{display:"flex",flexDirection:"column",gap:SPACING[8]}}>
          <button onClick={()=>copiar(linkScanTg,"tg")} style={{background:`${Q.cyan}18`,
            border:`1px solid ${Q.cyan}`,borderRadius:RADII.md,padding:"12px",cursor:"pointer",
            color:Q.cyan,fontSize:12,fontWeight:700,textAlign:"left",
            fontFamily:F_BODY}}>
            {copiado==="tg"?<><Icon name="circle-check" size={12}/> Copiado</>:<><Smartphone size={12}/> Copiar link de escáner (Telegram)</>}</button>
          <button onClick={()=>copiar(linkScanWeb,"web")} style={{background:`${Q.violet}18`,
            border:`1px solid ${Q.violet}`,borderRadius:RADII.md,padding:"12px",cursor:"pointer",
            color:Q.violet2,fontSize:12,fontWeight:700,textAlign:"left",
            fontFamily:F_BODY}}>
            {copiado==="web"?<><Icon name="circle-check" size={12}/> Copiado</>:<><Globe size={12}/> Copiar link de escáner (web)</>}</button>
        </div>
      </GCard>
  ...
  return(
    <div style={{maxWidth:520,margin:"0 auto",padding:"0 16px 40px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
        padding:"16px 0 12px"}}>
        <div style={{color:Q.text,fontWeight:800,fontSize:18,
          fontFamily:F_BODY}}><Star size={18}/> {agencia.name}</div>
        <button onClick={onLogout} style={{background:"transparent",
          border:`1px solid ${Q.border}`,borderRadius:RADII.md,padding:"8px 12px",
      ...
      <div style={{display:"flex",gap:SPACING[8],marginBottom:14}}>
        {[["panel",<><Icon name="chart-no-axes-combined" size={13}/> Mi panel</>],["combos",<><Zap size={13}/> Mis combos</>],["ia",<><Bot size={13}/> Combos IA</>],["escaner",<><Icon name="scan-line" size={13}/> Escáner</>]].map(([k,l])=>(
          <button key={k} onClick={()=>setTab(k)} style={{flex:1,
            ...
            padding:"12px",cursor:"pointer",color:tab===k?Q.cyan:Q.muted,
            fontSize:13,fontWeight:700,fontFamily:F_BODY}}>{l}</button>
        ))}
      </div>
      ...
      <GCard glow={Q.violet} style={{padding:SPACING[16],marginBottom:12,
        ...
        <div style={{display:"flex",flexDirection:"column",gap:SPACING[8]}}>
          <button onClick={()=>copiar(linkTelegram,"tg")} style={{background:`${Q.cyan}18`,
            ...
            color:Q.cyan,fontSize:12,fontWeight:700,textAlign:"left",
            fontFamily:F_BODY}}>
            {copiado==="tg"?<><Icon name="circle-check" size={12}/> Copiado</>:<><Smartphone size={12}/> Copiar link de Telegram</>}</button>
          <button onClick={()=>copiar(linkWeb,"web")} style={{background:`${Q.violet}18`,
            border:`1px solid ${Q.violet}`,borderRadius:RADII.md,padding:"12px",cursor:"pointer",
            color:Q.violet2,fontSize:12,fontWeight:700,textAlign:"left",
            fontFamily:F_BODY}}>
            {copiado==="web"?<><Icon name="circle-check" size={12}/> Copiado</>:<><Globe size={12}/> Copiar link web</>}</button>
        </div>
...
function InfluencerCombosIA({ agencia, onSesionExpirada }){
  ...
  return(
    <div>
      ...
      {!combos&&<div style={{color:Q.muted,textAlign:"center",padding:SPACING[20],
        fontFamily:F_BODY}}>Cargando...</div>}
      {combos&&combos.length===0&&<div style={{color:Q.muted,fontSize:12,textAlign:"center",
        padding:SPACING[20],fontFamily:F_BODY}}>No hay combos de la IA por ahora</div>}
          ...
          <GCard key={c.id} style={{padding:"12px 16px",marginBottom:8}}>
            <div onClick={()=>setAbierto(abierta?null:c.id)}
              style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                gap:SPACING[8],cursor:"pointer"}}>
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  fontFamily:F_BODY}}>{c.nombre}
                  {c.es_ia&&<span style={{background:`${Q.violet}33`,border:`1px solid ${Q.violet}`,
                    borderRadius:RADII.md,padding:"4px 8px",fontSize:12,color:Q.violet2,marginLeft:6}}><Bot size={12}/> IA</span>}
                  {c.expirado&&<span style={{background:`${Q.amber}22`,border:`1px solid ${Q.amber}`,
                    borderRadius:RADII.md,padding:"4px 8px",fontSize:12,color:Q.amber,marginLeft:6}}>expirado</span>}
                  {!c.expirado&&c.estado==="activo"&&<span style={{background:`${Q.green}22`,border:`1px solid ${Q.green}`,
                    borderRadius:RADII.md,padding:"4px 8px",fontSize:12,color:Q.green,marginLeft:6}}>activo</span>}</div>
              ...
              <div style={{marginTop:10,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:SPACING[8],marginTop:10}}>
                  <button onClick={()=>copiar(linkTg,`tg${c.id}`)} style={{background:`${Q.cyan}18`,
                    border:`1px solid ${Q.cyan}`,borderRadius:RADII.md,padding:"8px",cursor:"pointer",
                    color:Q.cyan,fontSize:12,fontWeight:700,textAlign:"left",
                    fontFamily:F_BODY}}>
                    {copiado===`tg${c.id}`?<><Icon name="circle-check" size={12}/> Copiado</>:<><Smartphone size={12}/> Link Telegram con tu firma</>}</button>
                  <button onClick={()=>copiar(linkWeb,`web${c.id}`)} style={{background:`${Q.violet}18`,
                    border:`1px solid ${Q.violet}`,borderRadius:RADII.md,padding:"8px",cursor:"pointer",
                    color:Q.violet2,fontSize:12,fontWeight:700,textAlign:"left",
                    fontFamily:F_BODY}}>
                    {copiado===`web${c.id}`?<><Icon name="circle-check" size={12}/> Copiado</>:<><Globe size={12}/> Link web con tu firma</>}</button>
                  <button onClick={()=>copiar(ref,`ref${c.id}`)} style={{background:"rgba(255,255,255,0.05)",
                    border:`1px solid ${Q.border}`,borderRadius:RADII.md,padding:"8px",cursor:"pointer",
                  ...
                  <button onClick={()=>copiar(textoCombo(c,ref,linkTg),`txt${c.id}`)}
                    ...
                    padding:"8px",cursor:"pointer",color:Q.green,fontSize:12,fontWeight:700,
                    textAlign:"left",fontFamily:F_BODY}}>
                    {copiado===`txt${c.id}`?<><Icon name="circle-check" size={12}/> Texto copiado</>:<><FileText size={12}/> Copiar texto para redes</>}</button>
                  <button onClick={()=>descargarPlacaCombo(c,ref,linkTg)}
                    style={{background:`${Q.gold}18`,border:`1px solid ${Q.gold}`,borderRadius:RADII.md,
                    padding:"8px",cursor:"pointer",color:Q.gold,fontSize:12,fontWeight:700,
                    textAlign:"left",fontFamily:F_BODY}}>
                    <ImageIcon size={12}/> Descargar placa (imagen)</button>
                  <button onClick={()=>setEditando(c)}
                    style={{background:`${Q.violet}18`,border:`1px solid ${Q.violet}`,borderRadius:RADII.md,
                    padding:"8px",cursor:"pointer",color:Q.violet2,fontSize:12,fontWeight:700,
                    textAlign:"left",fontFamily:F_BODY}}>
                    <Palette size={12}/> Editar imagen (crear placa)</button>
                </div>
              </div>
            )}
          </GCard>
          ...
          <GCard key={c.id} style={{padding:"12px 16px",marginBottom:8}}>
              ...
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontWeight:700,fontSize:13,
                  ...
                  {c.codigo&&<span style={{background:`${Q.cyan}22`,border:`1px solid ${Q.cyan}`,
                    borderRadius:RADII.md,padding:"4px 8px",fontSize:12,color:Q.cyan,marginLeft:6}}>{c.codigo}</span>}</div>
            ...
            {abierta&&(
              <div style={{marginTop:10,paddingTop:SPACING[12],borderTop:`1px solid ${Q.dim}`}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:SPACING[8],marginTop:6}}>
                  <button onClick={()=>copiar(linkTg,`tg${c.id}`)} style={{background:`${Q.cyan}18`,
                    border:`1px solid ${Q.cyan}`,borderRadius:RADII.md,padding:"8px",cursor:"pointer",
                    color:Q.cyan,fontSize:12,fontWeight:700,textAlign:"left",
                    fontFamily:F_BODY}}>
                  ...
                  <button onClick={()=>setEditando(c)}
                    ...
                    padding:"8px",cursor:"pointer",color:Q.violet2,fontSize:12,fontWeight:700,
                    textAlign:"left",fontFamily:F_BODY}}>
                    <Palette size={12}/> Editar imagen (Instagram)</button>
                </div>
              </div>
            )}
          </GCard>
  ...
  return(
    <GCard glow={Q.violet} style={{padding:SPACING[16],marginBottom:14}}>
      ...
      <input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Nombre del combo"
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
          borderRadius:RADII.md,padding:"8px 12px",color:Q.text,fontSize:16,marginBottom:8,
          fontFamily:F_BODY}}/>
      <input value={codigo} onChange={e=>setCodigo(e.target.value.toUpperCase())}
        placeholder="Código de salida (opcional, ej: JUANTIPS)"
        style={{width:"100%",background:"rgba(255,255,255,0.05)",border:`1px solid ${Q.border}`,
          borderRadius:RADII.md,padding:"8px 12px",color:Q.text,fontSize:16,marginBottom:12,
          fontFamily:F_BODY}}/>
        ...
        <GCard glow={Q.gold} style={{padding:SPACING[12],marginBottom:12,
          ...
          {picks.map((p,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
              padding:"4px 0",borderBottom:i<picks.length-1?`1px solid ${Q.dim}`:"none"}}>
              <div style={{minWidth:0,flex:1}}>
                <div style={{color:Q.text,fontSize:12,fontWeight:600,
                  fontFamily:F_BODY}}>{p.sel}</div>
                <div style={{color:Q.muted,fontSize:12}}>{p.home} vs {p.away}</div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            marginTop:10,paddingTop:SPACING[8],borderTop:`1px solid ${Q.violet}44`}}>
            <span style={{color:Q.muted,fontSize:12,textTransform:"uppercase",letterSpacing:1,
            ...
            {montosDe(agencia?.moneda).slice(1).map(m=>(
              <div key={m} style={{display:"flex",justifyContent:"space-between",padding:"4px 0"}}>
                <span style={{color:Q.muted,fontSize:12,fontFamily:F_BODY}}>
                  Apostás {ars(m)}</span>
                <span style={{color:Q.green,fontWeight:700,fontSize:12,
                  fontFamily:F_BODY}}>→ {ars(Math.round(m*oddTotal))}</span>
              </div>
            ))}
          </div>
        </GCard>
      ...
      <Btn label={proc?"GUARDANDO...":<><Save size={13}/> Crear combo</>} onClick={guardar}
      ...
      <div style={{marginTop:16}}>
        ...
        {!prematch&&<GCard style={{padding:SPACING[20],textAlign:"center"}}>
          <div style={{color:Q.muted,fontSize:12,fontFamily:F_BODY}}>
        ...
        {prematch&&prematch.length===0&&<GCard style={{padding:SPACING[20],textAlign:"center"}}>
          <div style={{color:Q.muted,fontSize:12,fontFamily:F_BODY}}>
            No hay eventos con cuotas ahora</div></GCard>}
        {filtrarDeportes(prematch||[], liga, busqueda).map(d=>(
          <GCard key={d.name} style={{padding:SPACING[16],marginBottom:10}}>
            <div style={{color:Q.violet2,fontWeight:700,fontSize:13,marginBottom:10,
              fontFamily:F_BODY}}>{d.icon} {d.name}</div>
            {(d.events||[]).map(ev=>(
              <div key={ev.id||(ev.h+ev.a)} style={{marginBottom:12,
                borderBottom:`1px solid ${Q.dim}`,paddingBottom:SPACING[12]}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6,gap:SPACING[8]}}>
                  <div style={{display:"flex",alignItems:"center",gap:SPACING[8],minWidth:0}}>
...
export default function QuartzAgencia(){
  ...
  // Las sesiones guardadas antes de que el login devolviera la moneda
  // no la tienen: se completa al arrancar. Sin esto, una agencia en USD
  // seguiría viendo montos en pesos hasta volver a entrar.
  useEffect(()=>{
    if(!agencia?.token || agencia?.moneda) return;
    fetch(`${API_URL}/api/agencias/me/cc`,
      {headers:authHeaders(agencia.token)})
      .then(r=>r.ok?r.json():null)
      .then(d=>{ if(d?.moneda) setAgencia({...agencia, moneda:d.moneda}); })
      .catch(()=>{});
    // eslint-disable-next-line
  },[agencia?.token]);
      ...
      {!agencia ? (
        ...
        <div style={{maxWidth:440,margin:"0 auto",padding:"40px 16px"}}>
          <div style={{textAlign:"center",marginBottom:20}}>
            <div style={{marginBottom:8}}><Lock size={44}/></div>
            <div style={{color:Q.text,fontWeight:800,fontSize:18,
              fontFamily:F_BODY}}>Bienvenido/a {agencia.name}</div>
          </div>
