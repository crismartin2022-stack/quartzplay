...
function ScreenCasino({ user }){
  ...
  useEffect(()=>{
    fetch(`${API}/api/casino/juegos`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(!d){ setJuegos([]); return; }
        // En el teléfono se muestran solo los que el proveedor marca
        // como aptos: los de escritorio se ven rotos en pantalla
        // chica y el jugador cree que la app falla.
        const aptos=(d.juegos||[]).filter(j=>j.movil!==false);
        setJuegos(aptos.length?aptos:(d.juegos||[]));
        setMarcas(d.marcas||[]);
      })
      .catch(()=>setJuegos([]));
  },[]);
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",
          ...
          {marcas.map(m=>(
            <button key={m} onClick={()=>{ setMarca(m); setPagina(1); }}
              style={{background:marca===m?`${Q.violet}33`:"transparent",
                border:`1px solid ${marca===m?Q.violet:Q.border}`,
                borderRadius:8,padding:"7px 13px",cursor:"pointer",
                color:marca===m?Q.cyan:Q.muted,fontSize:12,
                whiteSpace:"nowrap",flexShrink:0,
...
function MuroDesafios({ user, onCambio }){
  ...
  return(
    <div style={{padding:"14px 12px 20px"}}>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        ...
        <div onClick={()=>onNav("desafios")} style={{flex:1,
          cursor:"pointer",borderRadius:12,padding:"15px 13px",
          background:`linear-gradient(135deg,${Q.violet},${Q.cyan})`}}>
          <div style={{fontSize:22,marginBottom:3}}>🤝</div>
          <div style={{color:"#fff",fontWeight:800,fontSize:14,
