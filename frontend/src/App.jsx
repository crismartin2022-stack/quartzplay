...
function ScreenBuilder({ picks, onAdd, onQuitar, onLimpiar, onBet, onLocal, onNav }){
  ...
  const toggle=(ev,label,odd)=>{
    onAdd({id:ev.id,label,odd,h:ev.h||ev.home,a:ev.a||ev.away});
  };
  const toggleExpand=(id)=>setExpandedEvents(e=>({...e,[id]:!e[id]}));

  const tot=picks.length?picks.reduce((a,p)=>a*(parseFloat(p.odd)||1),1):1;
...
export default function QuartzSports(){
  ...
  const addPick=(pick)=>{
    setBuilderPicks(ps=>{
      // un pick por partido: reemplaza si ya hay uno del mismo evento
      const sinEse=ps.filter(p=>p.id!==pick.id);
      if(ps.some(p=>p.id===pick.id&&p.label===pick.label)) return sinEse;
      return [...sinEse, pick];
    });
  };
