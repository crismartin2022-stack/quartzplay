...
// Misma paleta que App.jsx: azul electrico + morado + dorado.
// El dorado se reserva para cuotas y plata.
const Q = {
  void:"#050914", deep:"#080E1F", dark:"#0D1530",
  surface:"#0D1530", card:"#111B3B",
  violet:"#2B6BFF", violet2:"#7B3FE4", cyan:"#5A8CFF",
  green:"#25D07A", pink:"#FF2D55", amber:"#FFA51F", gold:"#FFC531",
  ...
  return(
    <>
      <button onClick={abrir} style={{width:"100%",marginTop:8,
        background:"rgba(255,255,255,0.04)",border:`1px solid ${Q.cyan}44`,borderRadius:10,
...
function TeamLogo({ name, size=30 }){
  ...
  if(name&&!fallo) return(
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,
      background:"#0A1128",border:`1px solid ${Q.border}`,
      ...
      <div style={{display:"flex",gap:6,padding:"12px 20px 0",
        ...
        {[["armar","Armar"],["combos","Combos"],["mejorar","Bet Best"],["cashout","Cash out"]].map(([k,l])=>{
          ...
          <button key={k} onClick={()=>setSeccion(k)} style={{
            flex:bb?1.25:1,
            background: bb ? (on?`linear-gradient(145deg,#FFE07A,${Q.gold})`:`${Q.gold}1A`)
                           : (on?Q.violet:"#0A1128"),
