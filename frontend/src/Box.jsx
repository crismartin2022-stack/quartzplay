            ...
            {stats&&stats.tiene_stats&&stats.stats.map((st,i)=>{
              ...
              return(
                <div key={i} style={{marginBottom:14}}>
                  ...
                  <div style={{display:"flex",height:7,borderRadius:RADII.sm,overflow:"hidden",
                    background:ov(0.08)}}>
...
// El QR de esta terminal, fijo abajo a la derecha. El cliente lo
// escanea y arma su boleto en el teléfono mientras espera: llega al
// mostrador con el código listo y el cajero solo cobra.
// Los desafíos abiertos, en la pantalla del local. Solo lectura: el
// cajero no puede crear ni aceptar porque los desafíos son entre
// jugadores con cuenta y saldo propio.
//
// Sirve para generar conversación —"Juan puso 5.000 a que gana
// River"— y empujar a que la gente escanee el QR.
function DesafiosBox({ agenciaCode }){
  ...
  useEffect(()=>{
    let vivo=true;
    const traer=()=>{
      fetch(`${API}/api/p2p/abiertas?limite=12`)
        .then(r=>r.ok?r.json():null)
        .then(d=>{ if(vivo&&d?.activo) setLista(d.apuestas||[]); })
        .catch(()=>{});
    };
    traer();
    const t=setInterval(traer,90000);
    return()=>{ vivo=false; clearInterval(t); };
  },[]);

  // Rota solo: en una pantalla de local nadie va a tocar para pasar
  useEffect(()=>{
    if(lista.length<2) return;
    const t=setInterval(()=>setI(x=>(x+1)%lista.length),8000);
    return()=>clearInterval(t);
  },[lista.length]);

  if(!lista.length) return null;
  const d=lista[i%lista.length];

  return(
    <div style={{background:`linear-gradient(135deg,${Q.gold}14,${Q.violet}0D)`,
...
function QRTerminal({ agenciaCode }){
  const [term,setTerm]=useState(null);
  const [grande,setGrande]=useState(false);

  useEffect(()=>{
    if(!agenciaCode) return;
    // El Box no maneja sesión de agencia, así que la terminal se
    // consulta por el código del local.
    fetch(`${API}/api/box/${agenciaCode}/terminal`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{ if(d?.terminal) setTerm(d.terminal); })
      .catch(()=>{});
  },[agenciaCode]);

  if(!term) return null;

  const qr=(px)=>`https://api.qrserver.com/v1/create-qr-code/?size=${px}x${px}&margin=10&data=${encodeURIComponent(term.url)}`;
    ...
    return(
      <div onClick={()=>setGrande(false)} style={{position:"fixed",
        ...
        <div style={{color:Q.muted,fontSize:14,marginTop:10,
          textAlign:"center",lineHeight:1.5,maxWidth:420}}>
          Escaneá, armá tu boleto y vení al mostrador con el código.
          <br/>{term.nombre}</div>
        <div style={{color:Q.dim,fontSize:12,marginTop:18}}>
          Tocá para cerrar</div>
      </div>
    );
  }

  return(
    <div onClick={()=>setGrande(true)}
      style={{position:"fixed",right:14,
    ...
    <div style={{marginTop:10,paddingTop:10,borderTop:`1px solid ${Q.dim}`}}>
      ...
      <div style={{display:"flex",gap:6,marginTop:8}}>
        ...
        <button onClick={onQuitar} style={{background:`${Q.pink}18`,
          border:`1px solid ${Q.pink}`,borderRadius:RADII.md,padding:"8px 12px",
...
export default function Box(){
  ...
  // Arranca en pesos y se ajusta cuando llega la agencia con su moneda:
  // el dato del backend no está disponible en el primer render.
  const [monto,setMonto]=useState(montoInicial("ARS"));
  const [montoTocado,setMontoTocado]=useState(false);
  const [bbFeature,setBbFeature]=useState({activo:false,min_picks:2});
  // Generar combinada: en el box no hay cliente identificado, así que
  // el cupo es de la agencia entera. Sirve para evitar abuso, no como
  // límite por persona.
  const [genProc,setGenProc]=useState(false);
  const [genCupo,setGenCupo]=useState(null);
  ...
  const generarCombo=async(perfil)=>{
    ...
    try{
      const ru=await fetch(`${API}/api/combo-sugerido/usar`,{
        ...
        body:JSON.stringify({agencia_code:agenciaCode})});
      const du=await ru.json().catch(()=>({}));
      if(!ru.ok) throw new Error(du.detail||"Sin cupo");
      const r=await fetch(
        `${API}/api/combo-sugerido?perfil=${perfil}&agencia_code=${agenciaCode}`);
      const d=await r.json();
      if(!r.ok) throw new Error(d.detail||"No se pudo armar");
      setPicks(d.picks.map(p=>({id:p.event_id,event_id:p.event_id,
        h:p.h,a:p.a,home:p.h,away:p.a,sel:p.sel,odd:p.odd,
        market:"h2h",sport_key:p.sport_key,
        commence_time:p.commence_time})));
      fetch(`${API}/api/combo-sugerido/cupo?agencia_code=${agenciaCode}`)
        .then(x=>x.ok?x.json():null).then(x=>x&&setGenCupo(x)).catch(()=>{});
    }catch(e){ alert(e.message); }
    setGenProc(false);
  };

  // Si la agencia opera en otra moneda, el monto sugerido se adapta.
  // No se pisa lo que el cajero haya escrito a mano.
  useEffect(()=>{
    if(agencia?.moneda && !montoTocado) setMonto(montoInicial(agencia.moneda));
  },[agencia?.moneda, montoTocado]);
  useEffect(()=>{
    if(!agenciaCode) return;
  ...
  const togglePick=(ev,dep,label,odd)=>{
    ...
    setPicks(p=>{
      ...
      return[...w,{id,home:ev.h,away:ev.a,sel:label,odd,sport:dep,
        ...
        // La hora del partido viaja con el pick: la anulación la
        // necesita para saber si el evento está por empezar.
  ...
  const reiniciar=()=>{
    ...
    // El monto vuelve al sugerido de la moneda de la agencia, no a
    // 1000 fijo, que en dólares no tenía sentido.
    setMonto(montoInicial(agencia?.moneda)); setMontoTocado(false);
    setBusqueda(""); setLiga(null);
  };

  // Mantener selecciones: vuelve a armar sin borrar el boleto. Sirve
  // para que dos clientes jueguen lo mismo sin rehacer la búsqueda.
  const mantenerSelecciones=()=>{
    setSlip(null); setPantalla("armar"); setBusqueda(""); setLiga(null);
  };
    ...
    return(
      <div style={{...wrap,alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{maxWidth:460,width:"100%",textAlign:"center"}}>
          ...
          {picks.length>0&&(
            <button onClick={mantenerSelecciones} style={{width:"100%",
              background:`${Q.violet}22`,border:`1px solid ${Q.violet}`,
              borderRadius:RADII.lg,padding:"15px",cursor:"pointer",
          ...
          <button onClick={reiniciar} style={{width:"100%",
            ...
            border:"none",borderRadius:RADII.lg,padding:"16px",cursor:"pointer",
  ...
  return(
    <div style={wrap}>
        ...
        {[["armar","Armar"],["combos","Combos"],["mejorar","Bet Best"],["cashout","Cash out"]].map(([k,l])=>{
          ...
          <button key={k} onClick={()=>setSeccion(k)} style={{
            ...
            background: bb ? (on?`linear-gradient(145deg,#FFE07A,${Q.goldBg})`:`${Q.gold}1A`)
                           : (on?Q.violet:Q.inset),
            border:`1px solid ${bb?Q.gold:(on?Q.violet:Q.border)}`,borderRadius:RADII.md,
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 20px 120px",
              ...
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                ...
                <button onClick={consultarCO} disabled={!coCode||coProc}
                  style={{background:`${Q.gold}33`,border:`1.5px solid ${Q.gold}`,
                    borderRadius:RADII.md,padding:"0 18px",color:Q.gold,fontWeight:700,
              ...
              <div style={{background:`${Q.green}12`,border:`1px solid ${Q.green}`,
                ...
                {coHecho.destino==="cuenta"?(
                  ...
                  <div>
                    ...
                    <div style={{background:ov(0.06),border:`1px solid ${Q.gold}`,
                      borderRadius:RADII.lg,padding:"14px",color:Q.gold,fontWeight:900,fontSize:26,
                ...
                <button onClick={()=>{setCoHecho(null);setCoCode("");setCoValor(null);}}
                  style={{marginTop:16,background:`${Q.violet}22`,border:`1px solid ${Q.violet}`,
                    borderRadius:RADII.md,padding:"10px 24px",color:Q.violet2||Q.violet,
          ...
          <div>
            ...
            {imgsMejora.length>0&&(
              <button onClick={analizarImg} disabled={analizando} style={{width:"100%",
                ...
                border:"none",borderRadius:RADII.lg,padding:"14px",marginBottom:4,
              ...
              <div style={{marginTop:14}}>
                ...
                {resMejora.faltan_picks&&(
                  <div style={{background:`${Q.pink}12`,border:`1px solid ${Q.pink}66`,
                    borderRadius:RADII.md,padding:"9px 11px",marginBottom:10,color:Q.pink,
                ...
                {resMejora.picks_ok>0&&(
                  <button onClick={cargarMejora} style={{width:"100%",marginTop:6,
                    ...
                    border:"none",borderRadius:RADII.lg,padding:"14px",cursor:"pointer",
        ...
        {/* Buscador */}
        <AvisosBanner destino="box" agenciaCode={agenciaCode}/>
        ...
        {/* Armar una combinada al momento, para el cliente que no sabe
            qué jugar. El cupo es de la agencia porque acá no hay
            cliente identificado. */}
        {picks.length===0&&deportes&&deportes.length>0&&(
          <div style={{background:ov(0.04),border:`1px solid ${Q.border}`,
            borderRadius:RADII.lg,padding:"12px 14px",marginBottom:10}}>
              ...
              {[["seguro",<><Icon name="shield-check" size={13}/> Seguro</>],["equilibrado","⚖️ Medio"],
                ...
                <button key={k} onClick={()=>generarCombo(k)}
                  ...
                  style={{flex:1,background:ov(0.05),
                    ...
                    color:(genCupo&&genCupo.quedan<=0)?Q.dim:Q.cyan,
                  ...
                  {[{label:ev.h+" gana",odd:ev.odds?.L},
                    ...
                    <button key={opt.label}
                      ...
                      style={{flex:1,minWidth:0,
                        ...
                        border:`2px solid ${sel?Q.cyan:Q.border}`,borderRadius:RADII.lg,
      ...
      {/* QR para que el cliente arme su boleto en el teléfono */}
      <QRTerminal agenciaCode={agenciaCode}/>
          ...
          <div style={{maxWidth:720,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              ...
              <input type="number" inputMode="numeric" min="0" value={monto}
                ...
                style={{flex:1,minWidth:0,background:Q.inset,
                  border:`1px solid ${Q.border}`,borderRadius:RADII.md,padding:"9px 12px",
                  color:Q.text,fontSize:15,fontWeight:700,fontFamily:F_NUM}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",
