...
// ═══════════════════════════════════════════════════════════════
// Paleta: azul electrico (accion) + morado (profundidad) + dorado (valor).
// Regla: el dorado se usa SOLO para cuotas y plata. Nunca decorativo.
// Se conservan todas las claves viejas para no romper los ~900 usos existentes;
// cambia el valor, no el nombre.
const Q = {
  void:"#050914", deep:"#080E1F", dark:"#0D1530",
  surface:"#0D1530", card:"#111B3B",
...
function HojaConfirmar({ picks, stake, odd, user, onCerrar, onListo }){
  ...
  const Opcion = ({modo, titulo, detalle, disponible, color, deshabilitado}) => (
    <button disabled={!!enviando||deshabilitado} onClick={()=>jugar(modo)} style={{
      ...
      background:deshabilitado?"#0A1128":Q.surface,
    ...
    <div style={{flexShrink:0,background:Q.deep,borderTop:`1px solid ${Q.border}`,
      ...
      <div style={{flex:1,display:"flex",justifyContent:"center",position:"relative"}}>
        <button onClick={()=>onNav("mejorar")} aria-label="Bet Best" style={{
          ...
          background:activoBB
            ? `linear-gradient(145deg,#FFE07A,${Q.gold})`
            : `linear-gradient(145deg,${Q.gold},#E0A614)`,
          boxShadow:`0 6px 18px ${Q.gold}55`,
      ...
      <div onClick={()=>onNav("mejorar")} style={{
        ...
        <div style={{position:"relative"}}>
          ...
          <div style={{marginTop:12,display:"inline-flex",alignItems:"center",gap:7,
            background:Q.gold,borderRadius:7,padding:"9px 16px"}}>
