  ...
  return(
    <div>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[["resumen","📊 Resumen"],["movimientos","💸 Movs"],["apuestas","🎫 Apuestas"]].map(([k,l])=>(
        ...
        <div>
          <GCard glow={Q.violet} style={{padding:16,marginBottom:12,
            ...
            <div style={{color:Q.muted,fontSize:10,textTransform:"uppercase",
              ...
              fontFamily:"'Space Grotesk',system-ui"}}>Total del sistema</div>
      ...
      {vista==="apuestas"&&<>
        {filtroAg&&clientesF.length>0&&(
          <select value={filtroCli} onChange={e=>setFiltroCli(e.target.value)}
            style={{width:"100%",background:"rgba(255,255,255,0.05)",
              border:`1px solid ${filtroCli?Q.cyan:Q.border}`,borderRadius:9,
              padding:"9px 12px",color:Q.text,fontSize:14,marginBottom:10,
              fontFamily:"'Space Grotesk',system-ui"}}>
        ...
        {!filtroAg&&<div style={{color:Q.dim,fontSize:11,marginBottom:8,
          fontFamily:"'Space Grotesk',system-ui"}}>
          Elegí una agencia arriba para poder filtrar por cliente.</div>}
...
function FichaAgencia({ agencia, adminKey, onCambio, onNoAutorizado }){
  ...
  return(
    <GCard glow={Q.violet} style={{padding:14}}>
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[["cc","💰 Saldo"],["comision","% Comisión"],["editar","⚙️ Estado"]].map(([k,l])=>(
      ...
      {ver==="comision"&&(
        <ComisionAgencia agencia={agencia} adminKey={adminKey}
...
function CrearAgenciaAdmin({ adminKey, agencias, onListo, onNoAutorizado }){
  const [form,setForm]=useState({name:"",username:"",password:"",
    parent_code:"",pct_ggr:"",pct_ventas:"",moneda:"ARS"});
  const [msg,setMsg]=useState("");
  const [proc,setProc]=useState(false);
