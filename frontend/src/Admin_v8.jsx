...
function TabCierre({ adminKey, onNoAutorizado }){
  ...
  const cargar=async()=>{
    setCargando(true);
    try{
      const r=await fetch(`${API}/api/admin/cierre?desde=${desde}&hasta=${hasta}`,
  ...
  const exportar=()=>{
    fetch(`${API}/api/admin/cierre?desde=${desde}&hasta=${hasta}&export=csv`,
      {headers:adminHeaders(adminKey)})
  ...
  return(
    <div>
      <div style={{color:Q.text,fontWeight:700,fontSize:15,marginBottom:12,
        fontFamily:"'Space Grotesk',system-ui"}}>📊 Cierre del sistema</div>
