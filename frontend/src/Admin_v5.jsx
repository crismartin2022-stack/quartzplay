            ...
            <div style={{display:"flex",justifyContent:"space-between",
              ...
              <div style={{minWidth:0,flex:1}}>
                ...
                <div style={{color:Q.muted,fontSize:10,marginTop:1}}>
                  ...
                  {a.parent_code?` · cuelga de ${a.parent_code}`:" · raíz"}</div>
              </div>
...
function TabAgencias({ adminKey, onNoAutorizado }){
  ...
  const [ccProc,setCcProc]=useState(false);
  const [agencias,setAgencias]=useState([]);
  const [loading,setLoading]=useState(false);
  const [form,setForm]=useState({name:"",username:"",password:"",address:"",phone:"",parent_code:"",pct_ggr:"",pct_ventas:"",moneda:"ARS"});
  ...
  const crearAgencia=async()=>{
    ...
    try {
      ...
      if(r.ok){
        ...
        setForm({name:"",username:"",password:"",address:"",phone:"",parent_code:"",pct_ggr:"",pct_ventas:"",moneda:"ARS"});
        setShowForm(false);
        loadAgencias();
      } else {
  ...
  const cargarCC=async(signo)=>{
    ...
    if(!m||m<=0){ setCcMsg("Poné un monto"); return; }
    setCcProc(true); setCcMsg("");
    try{
      const r=await fetch(`${API}/api/admin/agencias/${selAgencia.code}/cc`,{
        method:"POST",headers:{"Content-Type":"application/json",...adminHeaders(adminKey)},
        body:JSON.stringify({monto:m*signo, detalle:signo>0?"Carga admin":"Descuento admin"}),
      });
      if(r.status===401){ onNoAutorizado(); return; }
      if(!r.ok){ const e=await r.json().catch(()=>({}));
        throw new Error(e.detail||`Error ${r.status}`); }
      const d=await r.json();
      setCcMsg(`✅ Saldo actualizado: ${ars(d.saldo_cc)}`);
      setSelAgencia(a=>({...a,saldo_cc:d.saldo_cc}));
      setCcMonto("");
    }catch(e){ setCcMsg("⚠️ "+e.message); }
    setCcProc(false);
  };
        ...
        <GCard glow={Q.gold} style={{padding:14,marginBottom:14,
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{color:Q.muted,fontSize:11,textTransform:"uppercase",letterSpacing:1,
              fontFamily:"'Space Grotesk',system-ui"}}>Saldo cuenta corriente</span>
            <span style={{color:(selAgencia.saldo_cc||0)>=0?Q.green:Q.red,fontWeight:900,fontSize:20,
              fontFamily:"'Space Grotesk',system-ui"}}>
              {ars(selAgencia.saldo_cc||0)} {selAgencia.moneda||"ARS"}</span>
          </div>
          <input value={ccMonto} onChange={e=>setCcMonto(e.target.value)}
            placeholder="Monto a cargar / descontar" inputMode="numeric"
            style={{width:"100%",background:"rgba(255,255,255,0.05)",
              border:`1px solid ${Q.border}`,borderRadius:9,padding:"10px 12px",
              color:Q.text,fontSize:16,marginBottom:8,
              fontFamily:"'Space Grotesk',system-ui"}}/>
          <div style={{display:"flex",gap:8}}>
            <Btn label="− Descontar" onClick={()=>cargarCC(-1)} color={Q.amber}
              outline full disabled={ccProc}/>
            <Btn label="+ Cargar saldo" onClick={()=>cargarCC(1)} color={Q.green}
              full disabled={ccProc}/>
          </div>
          {ccMsg&&<div style={{fontSize:12,marginTop:8,
            color:ccMsg.startsWith("✅")?Q.green:Q.red,
            fontFamily:"'Space Grotesk',system-ui"}}>{ccMsg}</div>}
          <div style={{color:Q.dim,fontSize:10,marginTop:8,
            fontFamily:"'Space Grotesk',system-ui"}}>
            El admin carga sin límite. La agencia usa este saldo para dar crédito a sus clientes.
          </div>
        </GCard>
    ...
    <div>
      ...
      {showForm&&(
        <GCard glow={Q.violet} style={{padding:18,marginBottom:14}}>
          ...
          <div style={{color:Q.muted,fontSize:10,textTransform:"uppercase",
            letterSpacing:1,margin:"8px 0 6px",
            fontFamily:"'Space Grotesk',system-ui"}}>Jerarquía y comisiones</div>
          <div style={{marginBottom:10}}>
            ...
            <select value={form.parent_code}
              ...
              style={{width:"100%",background:"rgba(255,255,255,0.05)",
                ...
                fontFamily:"'Space Grotesk',system-ui"}}>
              <option value="">— Raíz (cuelga del admin) —</option>
              {agencias.map(a=>(
                <option key={a.code} value={a.code}>{a.name} ({a.code})</option>
              ))}
            </select>
          ...
          <div style={{display:"flex",gap:8}}>
            <div style={{flex:1}}>
              <Input label="% GGR" value={form.pct_ggr}
                onChange={v=>setForm(f=>({...f,pct_ggr:v}))} placeholder="15"/>
            </div>
            <div style={{flex:1}}>
              <Input label="% Ventas" value={form.pct_ventas}
                onChange={v=>setForm(f=>({...f,pct_ventas:v}))} placeholder="5"/>
            </div>
          </div>
          <div style={{marginBottom:10}}>
            <div style={{color:Q.muted,fontSize:11,marginBottom:4,
              fontFamily:"'Space Grotesk',system-ui"}}>Moneda (solo si es raíz)</div>
            <select value={form.moneda}
              onChange={e=>setForm(f=>({...f,moneda:e.target.value}))}
              disabled={!!form.parent_code}
              style={{width:"100%",background:"rgba(255,255,255,0.05)",
                border:`1px solid ${Q.border}`,borderRadius:9,padding:"10px 12px",
                color:form.parent_code?Q.dim:Q.text,fontSize:15,
                fontFamily:"'Space Grotesk',system-ui"}}>
            ...
            {form.parent_code&&<div style={{color:Q.dim,fontSize:10,marginTop:3,
              fontFamily:"'Space Grotesk',system-ui"}}>
              Hereda la moneda de la agencia superior</div>}
          </div>
  ...
  return(
    <div style={{background:Q.void,minHeight:"100vh",
      ...
      <div style={{padding:"16px",maxWidth:620,margin:"0 auto",
        ...
        {tab==="arbol"    &&<TabArbol    adminKey={adminKey} onNoAutorizado={onLogout}/>}
        {tab==="clientes" &&<TabClientes adminKey={adminKey} onNoAutorizado={onLogout}/>}
