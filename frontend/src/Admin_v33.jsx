...
function TabEventos({ adminKey, onNoAutorizado }){
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/api/admin/agencias`,{headers:adminHeaders(adminKey)})
      ...
      .then(d=>d&&setAgs(d.agencias||[])).catch(()=>{});
    // eslint-disable-next-line
  },[]);
...
function TabRiesgo({ adminKey, onNoAutorizado }){
  ...
  useEffect(()=>{
    fetch(`${API}/api/admin/agencias`,{headers:adminHeaders(adminKey)})
      .then(r=>r.ok?r.json():null)
      .then(d=>{ if(d) setMonedas([...new Set(["ARS",
        ...(d.agencias||[]).map(a=>a.moneda).filter(Boolean)])]); })
      .catch(()=>{});
    // eslint-disable-next-line
  },[]);
  const [msg,setMsg]=useState("");
      ...
      <GCard glow={Q.violet} style={{padding:16,marginBottom:16}}>
        ...
        {monedas.length>1&&(
          <>
            ...
            <div style={{display:"flex",gap:6,marginBottom:10}}>
              {monedas.map(m=>(
                <button key={m} onClick={()=>setMonedaTope(m)} style={{flex:1,
                  background:monedaTope===m?`${Q.violet}33`
                    :"rgba(255,255,255,0.04)",
                  border:`1px solid ${monedaTope===m?Q.violet:Q.border}`,
                  borderRadius:8,padding:"9px 4px",cursor:"pointer",
                  color:monedaTope===m?Q.cyan:Q.muted,fontSize:12,
                  fontWeight:monedaTope===m?700:400,
                  fontFamily:"'Space Grotesk',system-ui"}}>{m}</button>
              ))}
            </div>
          </>
        ...
        {monedas.length>1&&(
          <div style={{color:Q.dim,fontSize:10.5,margin:"6px 0 10px",
            ...
            sin tope cargado usa el de pesos.</div>
        )}
...
function TabLimites({ adminKey, onNoAutorizado }){
  ...
  // Las monedas en uso salen de las agencias: no tiene sentido
  // ofrecer monedas que nadie opera.
  const monedasEnUso=[...new Set((agencias||[])
    .map(a=>a.moneda).filter(Boolean))];
  const MONEDAS=[...new Set(["ARS",...monedasEnUso])];
  const [msg,setMsg]=useState("");
  const [proc,setProc]=useState(false);
                ...
                {MONEDAS.map(m=>(
                  <button key={m} onClick={()=>setForm(f=>({...f,moneda:m}))}
                    ...
                    background:form.moneda===m?`${Q.violet}33`
                      :"rgba(255,255,255,0.04)",
                    border:`1px solid ${form.moneda===m?Q.violet:Q.border}`,
                    borderRadius:8,padding:"9px 4px",cursor:"pointer",
                    color:form.moneda===m?Q.cyan:Q.muted,fontSize:12,
